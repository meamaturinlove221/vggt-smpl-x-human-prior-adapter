...
APP_NAME = os.environ.get("VGGT_MODAL_V20200_APP_NAME", "vggt-v20200-visible-nonregression-connected-infill")
VOLUME_NAME = os.environ.get("VGGT_MODAL_V20200_VOLUME", "vggt-v20200-visible-nonregression-output")
REMOTE_REPO = PurePosixPath("/workspace/repo")
REMOTE_OUT = PurePosixPath("/v20200_out")
...
@app.function(
    image=image,
    gpu=os.environ.get("VGGT_MODAL_V20200_GPU", "A10G"),
    cpu=4.0,
    memory=24 * 1024,
    timeout=8 * 60 * 60,
    volumes={str(REMOTE_OUT): volume},
)
def run_v20200(steps: int = 300, max_points: int = 8192) -> dict[str, Any]:
    import os
    import shutil
    import subprocess
    import sys
    ...
    env["V20200_STEPS"] = str(steps)
    env["V20200_MAX_POINTS"] = str(max_points)
    env["PYTHONUTF8"] = "1"
    env["PYTHONIOENCODING"] = "utf-8"
    proc = subprocess.run(
        [sys.executable, "tools/V20200_visible_nonregression_connected_infill.py"],
        cwd=repo,
        env=env,
        text=True,
        capture_output=True,
    ...
    copied: list[str] = []
    for rel in [
        "reports/V20200000000000000000_visible_nonregression_connected_training_manifest.csv",
        "reports/V20200000000000000000_visible_nonregression_connected_scores.csv",
        "reports/V20200000000000000000_visible_nonregression_connected_decision.json",
        "reports/V20200000000000000000_runtime_environment.json",
        "boards/V20200000000000000000_visible_nonregression_connected_board.png",
        "boards/V20200000000000000000_visible_nonregression_connected_turntable_cross_section.png",
    ]:
        src = repo / rel
        if src.exists():
            dst = out_root / rel
            ...
            copied.append(rel)
    matrix_src = repo / "output" / "V20200000000000000000_visible_nonregression_connected_infill"
    matrix_dst = out_root / "output" / "V20200000000000000000_visible_nonregression_connected_infill"
    if matrix_src.exists():
        if matrix_dst.exists():
            shutil.rmtree(matrix_dst)
        shutil.copytree(matrix_src, matrix_dst)
        copied.append("output/V20200000000000000000_visible_nonregression_connected_infill")
    volume.commit()
    return {
        "created_at": now(),
...
def main(steps: int = 300, max_points: int = 8192) -> None:
    result = run_v20200.remote(steps=steps, max_points=max_points)
