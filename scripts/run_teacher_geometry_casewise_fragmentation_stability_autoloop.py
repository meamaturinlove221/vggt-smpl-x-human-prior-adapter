...
from scripts.casewise_fragmentation_stability_mutation_bank import (  # noqa: E402
    stage_a_casewise_source_mutations,
    stage_b_casewise_rehydrated_mutations,
    stage_c_casewise_label_mutations,
)
from scripts.score_correspondence_progress import compare_variant  # noqa: E402


FAMILY = "teacher_geometry_casewise_fragmentation_stability_audit"
FIRST_SHAPE = "stablelead_casewise_source_prune_consensus_margin_maskedhuman_v1"
PRIOR_FAMILY = "teacher_geometry_fragmentation_generalization_audit"
OLD_AUTOLOOP_ROOT = REPO_ROOT / "output" / "autoloop_teacher_geometry_fragmentation_generalization"
AUTLOOP_ROOT = REPO_ROOT / "output" / "autoloop_teacher_geometry_casewise_fragmentation_stability"
...
NEXT_FAMILY_BY_FAILURE = {
    ...
    "fragmentation_rebound": "teacher_geometry_casewise_source_poison_audit",
    "quality_regression": "teacher_geometry_casewise_consensus_quality_tradeoff_audit",
    "external_residue_rebound": "teacher_geometry_casewise_external_residue_audit",
    "no_movement": "teacher_geometry_casewise_source_poison_audit",
}
PROXY_VARIANTS = [
...
def write_truth_repair_report(previous: dict) -> Path:
    report_path = base.RESEARCH_ROOT / "casewise_fragmentation_stability_truth_repair_report.json"
    base.write_json(
        report_path,
        {
            ...
            "repair_statements": [
                "fragmentation-generalization already proved that global subset plus soft consensus can improve smoke and control metrics without opening cloud.",
                "that family still failed because benchmark-facing fragmentation remained unstable across cases even when coverage honesty was preserved.",
                "current family must restart from a fresh null baseline; prior best-state may be reused only as a seed subset candidate, never as the ranking baseline.",
            ],
            "repaired_truth": "The next honest move is casewise source pruning plus rehydrated consensus tuning, because the remaining wall is casewise fragmentation stability rather than evaluator honesty or global subset discovery.",
        },
    )
    return report_path
...
def write_packaging_files(report_path: Path) -> None:
    ...
    base.write_json(
        base.RESEARCH_ROOT / f"family_blueprint.{FAMILY}.json",
        {
            ...
            "notes": "Only evaluator/proxy/render autoloop is allowed. Training, dataset, trainer, and cloud training remain frozen.",
        },
    )
    base.write_json(
        base.RESEARCH_ROOT / f"candidate_patch_plan.{FAMILY}.json",
        {
            ...
            "write_surface": [
                ...
                "scripts/casewise_fragmentation_stability_mutation_bank.py",
                "scripts/run_teacher_geometry_casewise_fragmentation_stability_autoloop.py",
            ],
        },
    )
    base.write_json(
        base.RESEARCH_ROOT / f"next_manual_problem_draft.{FAMILY}.json",
        {
            ...
            "reason": "Current honest next move is a casewise fragmentation-stability autoloop because the previous family showed real smoke/control progress but benchmark-facing fragmentation still rebounded across cases.",
        },
    )
    base.write_json(
...
def select_control_rows(manifest: dict, smoke_rows: list[dict], *, limit: int = 6) -> list[dict]:
    rows = list(smoke_rows)
    seen = {base.case_id(row) for row in rows}
    for bucket in ("hero_cases", "benchmark_cases"):
...
def stage_failure_summary(compare: dict) -> str:
    ...
    if failure_class == "metric_truth_bug":
        return "casewise-fragmentation line still found a metric truth bug; the evaluator is not yet honest enough to rank casewise stability."
    if failure_class == "coverage_regression":
        return "casewise source pruning improved local agreement only by giving back too much visible human coverage or visible mass."
    if failure_class == "quality_regression":
        return "casewise pruning reduced fragmentation but regressed masked reconstruction quality, so the apparent progress is not honest."
    if failure_class == "external_residue_rebound":
        return "casewise pruning reduced fragmentation locally, but off-body or bottom-band residue rebounded on the broader set."
    if failure_class == "fragmentation_rebound":
        return "casewise pruning helped some smoke/control cases, but fragmentation still rebounded across harder cases and benchmark coverage remained unstable."
    return "bounded casewise-fragmentation mutations produced no honest movement on the benchmark-facing stability bottleneck."
...
def main() -> int:
    ...
    base.py_compile(
        [
            ...
            REPO_ROOT / "scripts" / "casewise_fragmentation_stability_mutation_bank.py",
            Path(__file__),
        ]
    )
    ...
    control_rows = select_control_rows(manifest, smoke_rows, limit=6)
    ...
    prior_subsets = []
    for candidate in [
        previous["old_best_state"].get("mutation", {}).get("source_subset", []),
        previous["old_best_local_state"].get("mutation", {}).get("source_subset", []),
    ]:
        if candidate:
            prior_subsets.append(list(candidate))
    for row in previous["old_stage_a_shortlist"].get("rows", [])[:3]:
        if row.get("source_subset"):
            prior_subsets.append(list(row["source_subset"]))

    stage_a_mutations = stage_a_casewise_source_mutations(source_count, prior_subsets=prior_subsets)
    ...
    for mutation in stage_a_mutations:
        ...
        ingest(report, decision_stage="stage_a_casewise_smoke")
    ...
    for short_item in shortlist:
        mutations = stage_b_casewise_rehydrated_mutations(
        ...
        for mutation in mutations:
            ...
            ingest(report, decision_stage="stage_b_casewise_smoke")
    ...
    for candidate in sorted(control_pass_reports, key=lambda item: ranking_key(item["best_compare"]))[:3]:
        mutations = stage_c_casewise_label_mutations(
        ...
        for mutation in mutations:
            ...
            ingest(report, decision_stage="stage_c_casewise_smoke")
            if report["best_compare"]["smoke_stage_accept"]:
                iter_idx += 1
    ...
    best_compare = best_state["compare"] if best_state is not None else {
        ...
        "mean_delta_source_id_switch_count_inside_fg": 0.0,
        "mean_delta_source_top1_spatial_fragmentation": 0.0,
    ...
    next_family = NEXT_FAMILY_BY_FAILURE.get(failure_class, "teacher_geometry_casewise_source_poison_audit")
    final_postmortem = {
        "checked_at": base.now_iso(),
        "family": FAMILY,
        "shape": FIRST_SHAPE,
