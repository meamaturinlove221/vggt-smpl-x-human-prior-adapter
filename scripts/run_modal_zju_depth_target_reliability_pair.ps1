param(
    ...
    [string]$ExpPrefix = "zju_depth_target_reliability_modal_pair",
...
if ([string]::IsNullOrWhiteSpace($OutputSubdirBase)) {
    $OutputSubdirBase = "depth_target_pairs/${runStamp}_$ExpPrefix"
}
...
$pairCfg = [ordered]@{
    ...
    min_depth_conf = 0.0
    baseline_min_depth_conf = 0.0
    candidate_min_depth_conf = $CandidateMinDepthConf
...
Write-Host "[modal-zju-depth-target-pair] repo_root=$repoRoot"
Write-Host "[modal-zju-depth-target-pair] throughput_profile=$ThroughputProfile"
Write-Host "[modal-zju-depth-target-pair] modal_gpu=$ModalGpu modal_cpu=$ModalCpu modal_memory_mb=$ModalMemoryMb"
Write-Host "[modal-zju-depth-target-pair] candidate_min_depth_conf=$CandidateMinDepthConf"
Write-Host "[modal-zju-depth-target-pair] output_subdir_base=$OutputSubdirBase"
Write-Host "[modal-zju-depth-target-pair] detach=$(-not $NoDetach)"

if ($DryRun) {
    Write-Host "[modal-zju-depth-target-pair] pair command:"
    Write-Host ($pairCommand -join " ")
    Write-Host "[modal-zju-depth-target-pair] cfg_json=$pairCfgJson"
    return
}
...
Write-Host "[modal-zju-depth-target-pair] done"
