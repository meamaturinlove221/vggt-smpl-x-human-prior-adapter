...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Summarize readiness for the tail manifest focal reinforcement family."
    )
...
def build_train_dataset_smoke(candidate_config_path: Path):
    ...
    return {
        ...
        "manifest_tail_focal_scale": float(cfg.loss.camera.sample_manifest_applied_focal_scale),
        "manifest_tail_focal_scale_train_only": bool(
            cfg.loss.camera.get("sample_manifest_applied_train_only", True)
        ),
    }
...
def main():
    ...
    payload = {
        ...
        "comparison_type": "tail_manifest_focal_reinforcement_readiness",
        ...
        "latest_tail_anchor_reserve_short_gate_deltas": {
        ...
        "hypothesis": (
            "The remaining short-gate camera tax after max-depth-anchor plus reserve hybridization is still tiny, "
            "stable, and concentrated in loss_FL rather than rotation. Because focal-off made camera much worse, "
            "the next bounded question is whether manifest-tail focal supervision should be modestly strengthened "
            "instead of weakened."
        ),
        ...
        "reasoning": [
            "This family is not another decoupling retry; it reinforces the focal component on manifest-tail samples using an already-supported camera-loss hook.",
            "The base contract stays fixed: refined hardtail manifest, reserve manifest, and hardtail max_depth_conf anchor all remain unchanged.",
            "The new bounded question is whether a small focal-only reinforcement can erase the residual camera/FL gap while preserving the strongest depth gains seen so far.",
        ],
    }
    output_json.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lines = [
        "# Tail Manifest Focal Reinforcement Readiness",
        "",
