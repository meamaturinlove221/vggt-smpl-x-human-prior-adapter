...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Summarize readiness for the tail dual-supervision rebalancing family."
    )
...
def build_train_dataset_smoke(candidate_config_path: Path):
    ...
    return {
        ...
        "hardtail_stream_min_supervised_views": int(base_hardtail.min_supervised_views),
        "reserve_stream_min_supervised_views": int(base_reserve.min_supervised_views),
        ...
        "hardtail_dual_supervised": len(hardtail_sample.get("supervised_camera_names", [])) == 2,
        ...
        "reserve_dual_supervised": len(reserve_sample.get("supervised_camera_names", [])) == 2,
    }
...
def main():
    ...
    payload = {
        ...
        "comparison_type": "tail_dual_supervision_rebalancing_readiness",
        ...
        "latest_tail_contract_viewset_replay_deltas": {
        ...
        "hypothesis": (
            "The remaining camera tax now looks like a single-supervised focal ambiguity problem: both hardtail "
            "and reserve manifests contain exactly one supervised view. Preserving anchor replay while raising "
            "tail streams to two supervised views may stabilize FL without giving up the tail depth gains."
        ),
        ...
        "reasoning": [
            "This family does not reopen view-set replay; it changes the tail supervision count while keeping manifest anchor replay fixed.",
            "The hardtail and reserve manifests are uniformly single-supervised, and the remaining validation regression is FL-dominated.",
            "The new bounded question is whether adding exactly one extra supervised geom view on tail streams stabilizes camera without erasing depth gains.",
        ],
    }
    output_json.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lines = [
        "# Tail Dual-Supervision Rebalancing Readiness",
        "",
