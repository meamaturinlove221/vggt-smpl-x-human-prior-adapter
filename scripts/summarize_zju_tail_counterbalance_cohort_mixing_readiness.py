...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Summarize readiness for the tail counterbalance cohort mixing family."
    )
...
def main():
    ...
    payload = {
        ...
        "comparison_type": "tail_counterbalance_cohort_mixing_readiness",
        ...
        "latest_tail_intrinsics_short_gate_deltas": {
        ...
        "hypothesis": (
            "The refined hardtail line keeps winning on depth while paying a small camera tax, and direct branch "
            "decoupling on the hardcase stream only makes camera worse. The next bounded question is whether the "
            "right fix is not to weaken hardcase supervision, but to companion the dominant B13 tail slice with a "
            "balanced reserve cohort from the under-enriched B1/B9/B5 anchors."
        ),
        ...
        "reasoning": [
            "This family is not another branch-decoupling retry and not another bucket-only retry; it keeps the refined hardtail stream and adds a distinct counterbalance reserve cohort.",
            "The reserve manifest is derived from refined-manifest-excluded near-tail rows and intentionally spreads exposure across B1/B9/B5 instead of repeating the B13-dominant slice.",
            "The next bounded question is whether companioning the dominant hardtail cohort with an anchor-balanced reserve stream preserves depth gains while fixing the short-gate camera tax.",
        ],
    }
    output_json.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lines = [
        "# Tail Counterbalance Cohort Mixing Readiness",
        "",
