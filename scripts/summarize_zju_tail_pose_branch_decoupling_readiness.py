...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Summarize readiness for the tail pose-branch decoupling family."
    )
...
def build_train_dataset_smoke(candidate_config_path: Path):
    ...
    return {
        ...
        "hardcase_candidate_config_camera_scale": float(cfg.loss.camera.sample_manifest_applied_scale),
    }
...
def main():
    ...
    payload = {
        ...
        "comparison_type": "tail_pose_branch_decoupling_readiness",
        ...
        "latest_tail_anchor_short_gate_deltas": {
        ...
        "hypothesis": (
            "The refined hardcase stream keeps producing stronger depth gains while paying a nearly fixed camera "
            "tax, even after conf-branch decoupling, source-pool tempering, and hardcase anchor stabilization. "
            "If the hardcase stream contributes depth supervision but no camera/T supervision, the persistent "
            "camera tax may disappear without giving up the refined-bucket depth signal."
        ),
        ...
        "reasoning": [
            "This family is not a bucket retry, not a source-pool retry, and not another anchor-policy retry; it changes only how the manifest-filtered hardcase stream contributes to camera/T supervision.",
            "The repo is ready because selection_sample_manifest_applied is now threaded into the composed batch and compute_camera_loss supports a manifest-conditioned train-only scale.",
            "The next bounded question is whether hardcase depth-only supervision removes enough short-gate camera tax to let the refined bucket depth gain survive into a formal gate win.",
        ],
    }
    output_json.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lines = [
        "# Tail Pose-Branch Decoupling Readiness",
        "",
