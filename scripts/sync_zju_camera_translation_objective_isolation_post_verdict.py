...
FAMILY_PLAN_JSON = RESEARCH_ROOT / "candidate_patch_plan.camera_translation_objective_isolation.json"
FAMILY_PLAN_MD = RESEARCH_ROOT / "candidate_patch_plan.camera_translation_objective_isolation.md"
DRAFT_JSON = RESEARCH_ROOT / "next_manual_problem_draft.camera_translation_objective_isolation.20260401.json"
DRAFT_MD = RESEARCH_ROOT / "next_manual_problem_draft.camera_translation_objective_isolation.20260401.md"
...
def main() -> int:
    ...
    if family != "camera_translation_objective_isolation":
        raise RuntimeError("candidate_verdict family must be camera_translation_objective_isolation for post-verdict sync.")
    ...
    updated_blueprint.update(
        {
            ...
            "why_now": "The single execution-ready losstisolation0 candidate has now been executed and closed with a dead_same_day short-gate verdict.",
            "latest_verdict": {
                "status": status,
                "reason": reason,
    ...
    family_readout["camera_translation_objective_isolation"] = {
    ...
    updated_family_stop["latest_family_outcomes"]["camera_translation_objective_isolation"] = {
    ...
    updated_research_status.update(
        {
            "checked_at": checked_at,
            "reason": (
                "No approved_problem.json is present; the single camera_translation_objective_isolation candidate has already been "
                "executed and closed as dead_same_day, so research remains idle in guard."
            ),
            "current_cloud_blocker": (
                "camera_translation_objective_isolation closed as dead_same_day at short_gate_10x5, and cloud must remain off. "
            ...
            "preferred_first_candidate_execution_note": "Do not auto-arm a next ticket. Define a new manual problem first.",
    ...
    updated_task_plan.update(
        {
            ...
            "task_mode_focus": "camera_translation_objective_isolation_dead_same_day_cloud_off",
        }
    )
    ...
    updated_task_plan["problem_definition_progress"].update(
        {
            "status": "camera_translation_objective_isolation_dead_same_day",
            "newest_boundary_fact": "The single execution-ready losstisolation0 candidate still failed the multi-metric stable-lead rule because T regressed strongly while depth terms stayed flat, so the family is closed for this round.",
    ...
    updated_task_plan["completed_this_round"] = upsert_by_key(
        ...
        {
            "id": "phase_38_camera_translation_execution_closed_dead_same_day",
            "status": "completed",
            "details": "Ran the single execution-ready losstisolation0 candidate and closed camera_translation_objective_isolation as dead_same_day at short_gate_10x5.",
        },
        "id",
    )
    updated_task_plan["completed_this_round"] = upsert_by_key(
        list(updated_task_plan.get("completed_this_round", []) or []),
        {
            "id": "phase_39_camera_translation_post_verdict_sync",
            "status": "completed",
            "details": "Synced translation family/task/watch truth to the dead_same_day post-verdict state and kept cloud off.",
        },
        "id",
    )
    ...
    blocked = upsert_by_key(
        blocked,
        {
            "id": "rearm_camera_translation_objective_isolation",
            "status": "blocked_by_formal_dead_same_day_verdict",
            "details": "The first camera_translation_objective_isolation ticket already spent the current family budget and may not be re-armed this round.",
        },
        "id",
    )
    ...
    updated_task_plan["fastest_next_path"] = [
        ...
        "Do not arm a second camera_translation_objective_isolation ticket.",
        "Keep cloud off.",
        "If another move is needed later, define a fresh manual problem first.",
    ]
    updated_task_plan["summary_conclusion"] = [
        "camera_translation_objective_isolation / stablelead_global_losst_isolation0 finished with a dead_same_day verdict at short_gate_10x5.",
        "Translation isolation did not remove the blocker: loss_camera only improved marginally while loss_T regressed sharply and depth terms stayed flat.",
        "Research is back in IDLE_GUARD, no approved problem remains, and cloud stays off.",
    ]
    updated_task_plan["current_state_notes"] = [
        "The single execution-ready camera_translation_objective_isolation candidate has been executed and closed for this round.",
    ...
    updated_watch["watch_conclusion"] = (
        "guard is green and research remains idle; camera_translation_objective_isolation closed as dead_same_day at short_gate_10x5, "
