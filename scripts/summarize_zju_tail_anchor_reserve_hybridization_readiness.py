...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Summarize readiness for the tail anchor reserve hybridization family."
    )
...
def build_train_dataset_smoke(candidate_config_path: Path):
    ...
    return {
        ...
        "hardtail_stream_manifest_applied": bool(base_hardtail.sample_manifest_applied),
        "reserve_stream_manifest_applied": bool(base_reserve.sample_manifest_applied),
        "hardtail_stream_anchor_policy": str(base_hardtail.source_anchor_policy),
        "reserve_stream_anchor_policy": str(base_reserve.source_anchor_policy),
...
def main():
    ...
    payload = {
        ...
        "comparison_type": "tail_anchor_reserve_hybridization_readiness",
        ...
        "latest_tail_counterbalance_short_gate_deltas": {
        ...
        "hypothesis": (
            "The strongest refined-tail ticket so far is the hardcase max-depth anchor variant, while the reserve "
            "cohort ticket shows the right non-dominant coverage shape but still leaves a small camera tax. The "
            "next bounded question is whether combining max-depth anchor stabilization on the hardtail stream with "
            "the reserve cohort can keep the larger depth gains while finally erasing the short-gate camera gap."
        ),
        "reserve_manifest_summary": {
            "target_count": int(reserve_manifest.get("target_count", 0)),
            "anchor_counts": reserve_manifest.get("anchor_counts", {}),
            "score_min": reserve_manifest.get("score_min"),
            "score_max": reserve_manifest.get("score_max"),
            "score_mean": reserve_manifest.get("score_mean"),
        },
        ...
        "reasoning": [
            "This family is not a same-family retry of tail_anchor_stabilization or tail_counterbalance_cohort_mixing; it combines their two strongest non-overlapping mechanisms in one bounded config-only contract.",
            "The hardtail stream switches only its anchor policy to max_depth_conf, while the reserve stream stays manifest-scoped and source-policy-fixed.",
            "The question is now about cross-cohort generalization balance, not about weakening supervision, changing loss routing, or reopening slot_3.",
        ],
    }
    output_json.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lines = [
        "# Tail Anchor Reserve Hybridization Readiness",
        "",
        ...
        "## Reserve Manifest",
        "",
        f"- reserve_manifest_summary: `{payload['reserve_manifest_summary']}`",
        "",
        "## Plumbing Smoke",
        "",
