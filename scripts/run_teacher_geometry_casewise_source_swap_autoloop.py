...
from scripts.casewise_source_swap_mutation_bank import (  # noqa: E402
    stage_a_source_swap_mutations,
    stage_b_rehydrated_mutations,
    stage_c_label_swap_mutations,
)
from scripts.score_correspondence_progress import compare_variant  # noqa: E402


FAMILY = "teacher_geometry_casewise_source_swap_audit"
FIRST_SHAPE = "stablelead_casewise_source_swap_consensus_margin_maskedhuman_v1"
PRIOR_FAMILY = "teacher_geometry_casewise_source_poison_audit"
OLD_AUTOLOOP_ROOT = REPO_ROOT / "output" / "autoloop_teacher_geometry_casewise_source_poison"
AUTLOOP_ROOT = REPO_ROOT / "output" / "autoloop_teacher_geometry_casewise_source_swap"
...
NEXT_FAMILY_BY_FAILURE = {
    ...
    "anchor_rebound": "teacher_geometry_dual_anchor_partition_audit",
    ...
    "no_movement": "teacher_geometry_dual_anchor_partition_audit",
}
PROXY_VARIANTS = [
...
def write_truth_repair_report(previous: dict) -> Path:
    report_path = base.RESEARCH_ROOT / "casewise_source_swap_truth_repair_report.json"
    base.write_json(
        report_path,
        {
            ...
            "repair_statements": [
                "casewise-source-poison already proved that bounded source pruning can reduce fragmentation and keep masked quality positive on some anchors.",
                "that family still failed because the honest smoke gate is a dual-anchor problem: B4 improved while B14 still rebounded on fg_connected_components.",
                "current family must restart from a fresh null baseline; prior best-state may seed source subsets, but it must not define the new family ranking baseline.",
            ],
            "repaired_truth": (
                "The next honest move is a casewise source-swap autoloop: keep the strong source subset seeds, "
                "but rank progress by the worst smoke row so both anchors must survive at once."
            ),
        },
    )
    return report_path
    ...
    base.write_json(
        base.RESEARCH_ROOT / f"candidate_patch_plan.{FAMILY}.json",
        {
            ...
            "write_surface": [
                ...
                "scripts/casewise_source_swap_mutation_bank.py",
                "scripts/run_teacher_geometry_casewise_source_swap_autoloop.py",
            ],
        },
    )
    base.write_json(
        base.RESEARCH_ROOT / f"next_manual_problem_draft.{FAMILY}.json",
        {
            ...
            "reason": (
                "Current honest next move is a casewise source-swap autoloop because source-poison pruning reduced some anchor fragmentation "
                "but could not make both smoke anchors pass together."
            ),
        },
    )
    base.write_json(
...
def stage_failure_summary(compare: dict) -> str:
    ...
    if failure_class == "metric_truth_bug":
        return "casewise source-swap line still found a metric truth bug; the evaluator is not honest enough to rank dual-anchor source swapping yet."
    if failure_class == "coverage_regression":
        return "casewise source swapping improved concentration only by giving back too much visible human coverage or visible mass."
    if failure_class == "quality_regression":
        return "casewise source swapping reduced fragmentation but regressed masked reconstruction quality, so the apparent progress is not honest."
    if failure_class == "external_residue_rebound":
        return "casewise source swapping reduced fragmentation locally, but off-body or bottom-band residue rebounded on the broader set."
    if failure_class == "anchor_rebound":
        return "casewise source swapping helped one smoke anchor, but the worst anchor still rebounded on fragmentation or largest visible component coverage."
    return "bounded casewise-source-swap mutations produced no honest dual-anchor movement on the smoke-facing stability bottleneck."
...
def main() -> int:
    ...
    base.py_compile(
        [
            ...
            REPO_ROOT / "scripts" / "casewise_source_swap_mutation_bank.py",
            Path(__file__),
        ]
    )
    ...
    stage_a_mutations = stage_a_source_swap_mutations(source_count=source_count, prior_best_subset=prior_best_subset)
    ...
    for mutation in stage_a_mutations:
        ...
        ingest(report, decision_stage="stage_a_source_swap_smoke")
    ...
    for short_item in shortlist:
        ...
        for mutation in mutations:
            ...
            ingest(report, decision_stage="stage_b_source_swap_rehydrated")
    ...
    for base_item in stage_c_candidates:
        mutations = stage_c_label_swap_mutations(
        ...
        for mutation in mutations:
            ...
            ingest(report, decision_stage="stage_c_source_swap_label")
    ...
    derived_next = NEXT_FAMILY_BY_FAILURE.get(failure_class, "teacher_geometry_dual_anchor_partition_audit")
    final_postmortem = {
        "checked_at": base.now_iso(),
        "family": FAMILY,
        "shape": FIRST_SHAPE,
