...
VALIDATION_MD = OUTPUT_ROOT / "execution_prep_baseline_validation.camera_objective_coupling_rebalancing.20260402.md"
...
def validate_signature_and_return_contract(loss_module, gt_pose):
    ...
    for name in ("loss_fl_isolation_scale", "loss_t_isolation_scale"):
        if name not in sig.parameters:
            raise AssertionError(f"{name} missing from compute_camera_loss signature")
        if sig.parameters[name].default != 1.0:
            raise AssertionError(f"{name} default is not identity")
...
def validate_joint_scale_only_affects_total_camera(loss_module, gt_pose):
    batch = make_batch(batch_size=2)
    ...
    scaled = loss_module.compute_camera_loss(
        ...
        weight_rot=1.0,
        weight_focal=0.5,
        loss_t_isolation_scale=1.05,
        loss_fl_isolation_scale=0.9,
    )
    assert_close(base["loss_T"], scaled["loss_T"], message="loss_T readout changed under coupling scales")
    assert_close(base["loss_R"], scaled["loss_R"], message="loss_R changed under coupling scales")
    assert_close(base["loss_FL"], scaled["loss_FL"], message="loss_FL changed under coupling scales")
    expected_scaled = base["loss_T"] * 1.05 + base["loss_R"] + base["loss_FL"] * 0.9 * 0.5
    assert_close(scaled["loss_camera"], expected_scaled, message="joint scaled loss_camera formula mismatch")
    return {
        "name": "joint_scale_only_affects_loss_camera_total",
        "status": "pass",
        "details": {
...
def render_md(payload: dict) -> str:
    lines = [
        "# Execution-Prep Baseline Validation: Camera Objective Coupling Rebalancing (2026-04-02)",
        "",
...
def main() -> int:
    ...
    validation_cases = [
        ...
        validate_joint_scale_only_affects_total_camera(loss_module, gt_pose),
    ]
    ...
    payload = {
        ...
        "summary": (
            "The reviewed joint FL/T camera-object coupling knobs passed stronger local validation: both hooks preserve "
            "identity defaults, the default formula is unchanged, and the coupled FL relief + T boost only changes total loss_camera aggregation."
        ),
        "environment_note": {
        ...
        "next_requirement": "decide_validated_joint_camera_object_coupling_knobs_can_enter_execution_ready_packaging",
    }
