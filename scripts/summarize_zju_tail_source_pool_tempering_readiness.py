...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Summarize readiness for the tail source-pool tempering family."
    )
...
def build_train_dataset_smoke(candidate_config_path: Path):
    ...
    default_sample = base_default.get_data(seq_index=0, img_per_seq=4)
    hardcase_sample = base_hardcase.get_data(seq_index=0, img_per_seq=4)
    return {
        ...
        "default_sample_source_view_pool_train_probability": float(
            default_sample.get("selection_source_view_pool_train_probability", 1.0)
        ),
        "hardcase_sample_source_view_pool_train_probability": float(
            hardcase_sample.get("selection_source_view_pool_train_probability", 1.0)
        ),
...
def main():
    ...
    payload = {
        ...
        "comparison_type": "tail_source_pool_tempering_readiness",
        ...
        "latest_tail_conf_short_gate_deltas": {
        ...
        "hardcase_source_pool_train_probability": 0.5,
        "hypothesis": (
            "The refined bucket line and both branch-local decoupling lines all kept the same small camera tax, "
            "which points away from a specific depth branch and toward the hardcase exposure contract itself. "
            "Tempering source_view_pool_train_probability only on the hardcase stream may preserve the depth gains "
            "from refined hard examples while reducing the source-pool stochasticity that leaks into camera loss."
        ),
        ...
        "reasoning": [
            "This family is not a bucket retry or a source-policy global retry: it keeps the refined hardtail mix and current eval source policy fixed while changing only the hardcase stream's train-time source-pool probability.",
            "The first candidate remains config-only because the dataset already supports per-stream source_view_pool_train_probability and the repo already contains a trainmix50 precedent.",
            "The next bounded question is whether hardcase-only source-pool tempering removes enough camera tax to let the refined bucket depth gain survive into a formal gate win.",
        ],
    }
    output_json.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lines = [
        "# Tail Source-Pool Tempering Readiness",
        "",
        ...
        "",
        "## Latest Tail-Conf Deltas",
        "",
        f"- latest_tail_conf_short_gate_deltas: `{payload['latest_tail_conf_short_gate_deltas']}`",
        "",
        "## Plumbing Smoke",
        "",
