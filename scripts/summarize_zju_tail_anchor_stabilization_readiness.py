...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Summarize readiness for the tail anchor stabilization family."
    )
...
def build_train_dataset_smoke(candidate_config_path: Path):
    ...
    for seq_index in range(12):
        sample = base_hardcase.get_data(seq_index=seq_index, img_per_seq=4)
        anchor = str(sample.get("selection_anchor_camera"))
        anchors.append(anchor)
        score_map = sample.get("supervised_view_quality_scores", {})
        score_value = score_map.get(anchor)
        if score_value is not None:
            anchor_scores.append(float(score_value))
    ...
    return {
        ...
        "hardcase_stream_source_anchor_policy": str(base_hardcase.source_anchor_policy),
        "hardcase_anchor_histogram_probe12": dict(Counter(anchors)),
        "hardcase_anchor_quality_probe12": {
            "mean": float(sum(anchor_scores) / len(anchor_scores)) if anchor_scores else None,
            "min": float(min(anchor_scores)) if anchor_scores else None,
            "max": float(max(anchor_scores)) if anchor_scores else None,
        },
    }
...
def main():
    ...
    payload = {
        ...
        "comparison_type": "tail_anchor_stabilization_readiness",
        ...
        "latest_tail_source_pool_short_gate_deltas": {
        ...
        "hypothesis": (
            ...
            "tempering, which points to supervision-case quality rather than a particular depth branch or rawpool "
            "stochasticity. Stabilizing the hardcase anchor to the per-frame max-depth-conf view may preserve hard "
            "depth gains while reducing low-quality anchor-induced camera drift."
        ),
        ...
        "reasoning": [
            "This family is not a bucket retry or a source-policy global retry: it keeps the refined hardtail mix and current eval source policy fixed while changing only the hardcase stream's anchor policy.",
            "The first candidate remains config-only because the dataset already supports source_anchor_policy=max_depth_conf on a per-stream basis.",
            "The next bounded question is whether hardcase-only anchor stabilization removes enough camera tax to let the refined bucket depth gain survive into a formal gate win.",
        ],
    }
    output_json.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lines = [
        "# Tail Anchor Stabilization Readiness",
        "",
