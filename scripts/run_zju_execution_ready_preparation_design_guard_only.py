...
OUTPUT_ROOT = RESEARCH_ROOT / "preparation_design_guard_only"
...
MODE = "PREPARATION_DESIGN_GUARD_ONLY"
EXPECTED_NEXT = "manual_review_execution_ready_preparation_design_before_any_execution_ready_status_decision"
ALLOWED_PROCESS_MARKERS = (
    "run_zju_execution_ready_preparation_design_guard_only.py",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run the preparation-design overnight guard.")
...
def detect_drift(smoke: dict, baseline_hash: str):
    ...
    if task.get("task_mode_focus") != "execution_ready_preparation_design_packaged_cloud_off":
        drifts.append("task_plan.task_mode_focus")
    if ((task.get("problem_definition_progress", {}) or {}).get("status") != "execution_ready_preparation_design_packaged_not_execution_ready"):
    ...
    if design.get("next_requirement") != EXPECTED_NEXT:
        drifts.append("preparation_design.next_requirement")
    ...
    return drifts, research, task, watch, design, modal, repo, lead


def render_md(report):
    lines = [
        "# Overnight Preparation Design Guard Report",
        "",
...
def run_cycle(args, session_dir: Path, cycle_index: int, baseline_hash: str, report_json: Path, report_md: Path):
    ...
    try:
        ...
        smoke = loss_smokes()
        drifts, research, task, watch, design, modal, repo, lead = detect_drift(smoke, baseline_hash)
    ...
    report = {
        ...
        "review_readiness": "still_manual_review_only_but_discussion_ready" if not drifts and not error else "",
...
def main():
    ...
    write_json(session_dir / "baseline.json", {"checked_at": iso_now(), "loss_py_sha256": baseline_hash, "expected_lead_config": expected_lead()})
    report_json = RESEARCH_ROOT / f"overnight_preparation_design_guard_report.{FAMILY}.{day_tag()}.json"
    report_md = RESEARCH_ROOT / f"overnight_preparation_design_guard_report.{FAMILY}.{day_tag()}.md"
    ...
    print(json.dumps({"session_dir": str(session_dir.resolve()), "report_json": str(report_json.resolve()), "report_md": str(report_md.resolve()), "status": report.get("status", "")}, ensure_ascii=False, indent=2))
    return 0 if report.get("status") == "NO_DRIFT" else 0
