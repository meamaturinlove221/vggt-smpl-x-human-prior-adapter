...
FIRST_CANDIDATE_SHAPE = "maxdepthanchor_plus_balance_reserve_focal1125"
FIRST_CANDIDATE_CONFIG = (
    ...
    "focalscale1125_minimal.yaml"
)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Package the tail manifest focal reinforcement family into manual-approval artifacts."
    )
...
def build_seed():
    return {
        "approved": False,
        "approved_at": "",
        "problem_id": "promoted_tail_manifest_focal_reinforcement_v1",
        "problem_title": "Promoted-lead tail manifest focal reinforcement",
        "family": "tail_manifest_focal_reinforcement",
        ...
        "problem_statement": (
            "Design exactly one genuinely new-family candidate that keeps the current strongest tail contract fixed, "
            "then modestly reinforces focal supervision only on manifest-tail samples so the residual short-gate "
            "camera tax might disappear without giving up the depth gains."
        ),
        "why_genuinely_new": (
            "This is a focal-reinforcement family rather than another decoupling or bucket family. It changes only "
            "the manifest-tail focal component scale while keeping the three-stream contract and anchor policy fixed."
        ),
        "why_not_reopening_frozen_family": (
            "It does not reopen tail_intrinsics_branch_decoupling, tail_anchor_stabilization, tail_counterbalance_"
            "cohort_mixing, or tail_anchor_reserve_hybridization as same-family retries. The new question is whether "
            "the missing ingredient is stronger focal supervision on the manifest-tail cohorts."
        ),
        "first_candidate_hint": (
            "Launch only the maxdepthanchor_plus_balance_reserve_focal1125 candidate: keep the 80k/10k/10k three-"
            "stream contract fixed, keep the hardtail stream on max_depth_conf, and set train-only manifest-tail "
            "focal scale to 1.125."
        ),
        ...
        "first_candidate_knobs": {
            ...
            "manifest_tail_focal_scale": 1.125,
            "manifest_tail_focal_scale_train_only": True,
...
def render_draft_md(draft: dict) -> str:
    lines = [
        "# Tail Manifest Focal Reinforcement Draft (2026-03-30)",
        "",
...
def render_plan_md(plan: dict) -> str:
    lines = [
        "# Candidate Patch Plan: Tail Manifest Focal Reinforcement",
        "",
...
def main():
    ...
    blueprint = {
        ...
        "why_now": (
            "The strongest tail contract now uses max-depth-anchor hardtail plus reserve coverage, and the "
            "remaining short-gate miss is still a tiny FL-dominated camera gap. The next bounded question is "
            "whether that gap is fixed by modest focal reinforcement rather than another decoupling move."
        ),
        "why_not_same_family_retry": (
            "This is not another reserve-only or anchor-only retry. It keeps the best current contract fixed and "
            "changes only the manifest-tail focal component scale."
        ),
        "signal_definition": (
            "Keep the three-stream tail contract and hardtail max_depth_conf anchor fixed, and raise only the "
            "train-only manifest-tail focal scale above 1.0."
        ),
        "scope_definition": "config only; do not rewrite loss routing, source policy, sampler logic, or cloud flow",
        ...
        "first_candidate_execution_note": (
            "This first focal-reinforcement candidate is repo-ready because the manifest-conditioned focal scale "
            "hook already exists in compute_camera_loss."
        ),
    ...
    draft = {
        ...
        "why_now": [
            "The best current tail contract now combines max-depth-anchor hardtail with reserve coverage.",
            "The remaining short-gate miss is still small and FL-dominated rather than rotation-dominated.",
            "The next bounded question is whether modest focal reinforcement on manifest-tail samples closes that gap.",
        ],
    ...
    plan = {
        ...
        "first_candidate_shape": seed["first_candidate_shape"],
        "first_candidate_config": seed["first_candidate_config"],
        "arm_command": "python scripts/arm_zju_source_policy_approved_problem.py --seed tail_manifest_focal_reinforcement",
