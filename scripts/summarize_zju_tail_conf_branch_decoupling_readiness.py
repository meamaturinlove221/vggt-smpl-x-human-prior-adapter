...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Summarize readiness for the tail conf-branch decoupling family."
    )
...
def main():
    ...
    quality_window = soft_readiness.get("selected_quality_window", {})
    latest_short = latest_verdict.get("short_gate_vs_lead", {})
    payload = {
        "checked_at": datetime.now().astimezone().isoformat(),
        "comparison_type": "tail_conf_branch_decoupling_readiness",
        ...
        "dominant_contract_anchor_camera": soft_readiness.get("dominant_contract_anchor_camera", "Camera_B13"),
        "selected_quality_window": quality_window,
        "latest_hybrid_short_gate_deltas": {
        ...
        "hypothesis": (
            "The refined bucket family improved conf_depth and reg_depth but paid a camera plus grad-depth tax. "
            "Because respect_conf_mask_in_grad_conf is already enabled, routing the dominant B13 quality-band "
            "slice out of conf-depth supervision while leaving the regression branch unchanged may preserve more "
            "of the bucket depth gain while removing the conf-driven camera tax."
        ),
        ...
        "reasoning": [
            "This family is not bucket-only, soft-only, or hybrid-soft retry: it keeps the refined hardtail mix but changes only the dominant slice's conf branch by using the existing conf-mask hook.",
            "The first candidate remains config-only because both ingredients are already live in the repo: manifest-aware two-stream mixing and anchor-conditioned conf-mask drop.",
            "The next bounded question is whether conf-branch-only decoupling removes the short-gate camera tax while keeping enough of the refined bucket depth gain to beat the current lead.",
        ],
    }
    output_json.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lines = [
        "# Tail Conf-Branch Decoupling Readiness",
        "",
        ...
        "",
        "## Latest Hybrid Deltas",
        "",
        f"- latest_hybrid_short_gate_deltas: `{payload['latest_hybrid_short_gate_deltas']}`",
        "",
        "## Plumbing Smoke",
        "",
