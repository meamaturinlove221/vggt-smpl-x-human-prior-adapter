...
FIRST_CANDIDATE_CONFIG = (
    ...
    "gradconfmask_contractsegmentstratifiedhardtailplusreserve8to1to1_manifestanchorreplay_minimal.yaml"
)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Package the tail contract anchor replay family into manual-approval artifacts."
    )
...
def build_seed():
    return {
        "approved": False,
        "approved_at": "",
        "problem_id": "promoted_tail_contract_anchor_replay_v1",
        "problem_title": "Promoted-lead tail contract anchor replay",
        ...
        "problem_statement": (
            ...
            "contract fixed, then replays the manifest-prescribed anchor camera on both tail streams so the "
            "training contract matches the promoted residual evidence more faithfully."
        ),
        "why_genuinely_new": (
            "This is a contract-replay family rather than another loss-side or ratio-side family. It changes how "
            "manifest entries are interpreted into anchor selection."
        ),
        "why_not_reopening_frozen_family": (
            "It does not reopen tail_anchor_reserve_hybridization or any focal-scaling family as same-family retries. "
            "The new question is whether the missing ingredient is contract-faithful anchor replay, not another loss knob."
        ),
        "first_candidate_hint": (
            "Launch only the manifest_anchor_replay_on_tail_streams candidate: keep the 80k/10k/10k contract fixed, "
            "and for both hardtail and reserve streams choose the anchor camera from each manifest entry's "
            "`promoted_anchor_camera` field."
        ),
...
def render_draft_md(draft: dict) -> str:
    lines = [
        "# Tail Contract Anchor Replay Draft (2026-03-30)",
        "",
...
def render_plan_md(plan: dict) -> str:
    lines = [
        "# Candidate Patch Plan: Tail Contract Anchor Replay",
        "",
...
def main():
    ...
    blueprint = {
        ...
        "why_now": (
            "Recent reserve and focal families all looked like no-ops, and the remaining clue is that manifests "
            "carry anchor identities that were not yet replayed into training. The next bounded question is whether "
            "contract-faithful anchor replay is the missing mechanism."
        ),
        "why_not_same_family_retry": (
            "This is not another loss-side cousin. It preserves the best current contract and changes only how "
            "manifest entries determine anchor selection."
        ),
        "signal_definition": (
            "Keep the three-stream tail contract fixed and use each manifest entry's promoted_anchor_camera as the "
            "selected anchor on the hardtail and reserve streams."
        ),
        ...
        "first_candidate_execution_note": (
            "This first contract-replay candidate is repo-ready because the manifests already carry promoted_anchor_camera "
            "and the dataset can now replay it."
        ),
    ...
    draft = {
        ...
        "why_now": [
            "Tail manifests already carry promoted_anchor_camera.",
            "Previous reserve-related tickets likely underdelivered because training only used those manifests as frame filters.",
            "The next bounded question is whether replaying the manifest anchor is the missing mechanism.",
        ],
    ...
    plan = {
        ...
        "first_candidate_shape": seed["first_candidate_shape"],
        "first_candidate_config": seed["first_candidate_config"],
        "arm_command": "python scripts/arm_zju_source_policy_approved_problem.py --seed tail_contract_anchor_replay",
