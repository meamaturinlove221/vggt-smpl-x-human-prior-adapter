...
FIRST_CANDIDATE_SHAPE = "stratified_bucket_hardcase_trainmix50"
FIRST_CANDIDATE_CONFIG = (
    "training/config/"
    "zju_vggt_geom_unproject_source_policy_nearestplusuniformtail_rawpool_confdepth_dropworst_"
    "gradconfmask_contractsegmentstratifiedhardtailbucketmix4to1_hardcasetrainmix50_minimal.yaml"
)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Package the tail source-pool tempering family into manual-approval artifacts."
    )
...
def build_seed():
    return {
        "approved": False,
        "approved_at": "",
        "problem_id": "promoted_tail_source_pool_tempering_v1",
        "problem_title": "Promoted-lead tail source-pool tempering",
        "family": "tail_source_pool_tempering",
        ...
        "problem_statement": (
            ...
            "mix fixed, then tempers source_view_pool_train_probability only on the hardcase stream to 0.5 so "
            "the depth gains might survive without the short-gate camera tax."
        ),
        "why_genuinely_new": (
            "This is an exposure-contract family: not bucket-only, not branch-local, and not a global source-"
            "policy reopen. It changes only the hardcase stream's train-time source-pool stochasticity."
        ),
        "why_not_reopening_frozen_family": (
            ...
            "tail_exposure_balancing, or tail_conf_branch_decoupling as same-family retries. The new question is "
            "whether the remaining camera tax is caused by hardcase-specific source-pool randomness."
        ),
        "first_candidate_hint": (
            "Launch only the stratified_bucket_hardcase_trainmix50 candidate: keep the refined 4:1 contract-"
            "segment bucket, leave the default stream untouched, set the hardcase stream source_view_pool_train_"
            "probability to 0.5, and do not branch into ratio tweaks or branch-local cousins."
        ),
        ...
        "first_candidate_knobs": {
            ...
            "default_stream_source_view_pool_train_probability": 1.0,
            "hardcase_stream_source_view_pool_train_probability": 0.5,
        },
        "avoid_patterns": [
            "bucket-only retry",
            "branch-local retry",
        ...
        "mutation_dsl": {
            "allow_tail_source_pool_tempering": True,
            "keep_refined_hardtail_mix_fixed": True,
            "keep_current_source_policy_eval_fixed": True,
            "allow_hardcase_stream_source_pool_probability_only": True,
            "require_refined_hardtail_manifest": True,
            "require_hardcase_trainmix50": True,
...
def render_draft_md(draft: dict) -> str:
    lines = [
        "# Tail Source-Pool Tempering Draft (2026-03-30)",
        "",
...
def render_plan_md(plan: dict) -> str:
    lines = [
        "# Candidate Patch Plan: Tail Source-Pool Tempering",
        "",
...
def main():
    ...
    blueprint = {
        ...
        "why_now": (
            "The refined bucket line and both branch-local decoupling lines all preserved the same small camera "
            "tax, which points away from a specific depth branch and toward the hardcase exposure contract "
            "itself. The next bounded question is whether hardcase-only source-pool randomness is the tax."
        ),
        "why_not_same_family_retry": (
            "This is explicitly not another bucket-only or branch-local retry. It keeps the refined mix and "
            "changes only the hardcase stream's source_view_pool_train_probability."
        ),
        "signal_definition": (
            "Keep the refined 4:1 contract-segment bucket fixed and temper source_view_pool_train_probability "
            "only on the hardcase stream so the candidate can test whether source-pool stochasticity drives "
        ...
        "first_candidate_execution_note": (
            "This first exposure-contract candidate is repo-ready because the dataset already supports per-stream "
            "source_view_pool_train_probability and the refined manifest mix is already live."
        ),
    ...
    draft = {
        ...
        "why_now": [
            ...
            "The branch-local decoupling tickets changed depth-branch behavior materially but did not remove that camera tax.",
            "The next bounded question is whether the remaining camera tax comes from hardcase-specific source-pool stochasticity rather than the depth branch itself.",
        ],
        "evidence_refs": [
        ...
        "latest_tail_conf_short_gate_deltas": readiness.get("latest_tail_conf_short_gate_deltas", {}),
        "hardcase_source_pool_train_probability": readiness.get("hardcase_source_pool_train_probability", 0.5),
    }
    ...
    plan = {
        ...
        "first_candidate_shape": seed["first_candidate_shape"],
        "first_candidate_config": seed["first_candidate_config"],
        "arm_command": "python scripts/arm_zju_source_policy_approved_problem.py --seed tail_source_pool_tempering",
