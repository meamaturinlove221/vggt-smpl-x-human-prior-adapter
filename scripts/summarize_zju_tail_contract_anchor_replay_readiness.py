...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Summarize readiness for the tail contract anchor replay family."
    )
...
def _manifest_anchor_by_key(manifest_payload):
    mapping = {}
    for entry in list(manifest_payload.get("entries", [])):
        mapping[(str(entry["seq_name"]), int(entry["frame_id"]))] = str(entry.get("promoted_anchor_camera") or "")
    return mapping
...
def build_train_dataset_smoke(candidate_config_path: Path, hardtail_manifest: dict, reserve_manifest: dict):
    ...
    img_per_seq = int(OmegaConf.to_container(cfg.data.train.common_config, resolve=True).get("img_per_seq", 4))
    ...
    hardtail_expected = _manifest_anchor_by_key(hardtail_manifest).get(
        (str(hardtail_sample["entry_seq_name"]), int(hardtail_sample["entry_frame_id"])),
        "",
    )
    reserve_expected = _manifest_anchor_by_key(reserve_manifest).get(
    ...
    return {
        ...
        "hardtail_expected_anchor_camera": str(hardtail_expected),
        "reserve_sample_anchor_camera": str(reserve_sample.get("selection_anchor_camera", "")),
        "reserve_expected_anchor_camera": str(reserve_expected),
        "hardtail_anchor_match": str(hardtail_sample.get("selection_anchor_camera", "")) == str(hardtail_expected),
        "reserve_anchor_match": str(reserve_sample.get("selection_anchor_camera", "")) == str(reserve_expected),
    }
...
def main():
    ...
    payload = {
        ...
        "comparison_type": "tail_contract_anchor_replay_readiness",
        "family": "tail_contract_anchor_replay",
        ...
        "first_candidate_shape": "manifest_anchor_replay_on_tail_streams",
        "first_candidate_config": str(candidate_config_path.resolve()),
        "latest_tail_stream_selective_focal_reinforcement_deltas": {
        ...
        "hypothesis": (
            "The reserve and hardtail manifests were built from promoted contracts that include an anchor camera, "
            "but training has so far only used them as frame filters. Replaying the manifest-prescribed anchors on "
            "both tail streams may be the missing step that turns the no-op reserve family into a real camera-side change."
        ),
        ...
        "reasoning": [
            "This family is not another loss-side cousin. It preserves the best current three-stream contract and changes only how manifest entries are interpreted into anchor selection.",
            "The reserve manifest already carries promoted_anchor_camera, but previous tickets did not replay it during training.",
            "The new bounded question is whether contract-faithful anchor replay on tail streams removes the remaining camera tax while keeping the strongest depth gains.",
        ],
    }
    output_json.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lines = [
        "# Tail Contract Anchor Replay Readiness",
        "",
