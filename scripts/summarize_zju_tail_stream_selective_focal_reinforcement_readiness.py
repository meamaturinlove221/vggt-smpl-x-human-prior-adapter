...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Summarize readiness for the tail stream-selective focal reinforcement family."
    )
...
def build_train_dataset_smoke(candidate_config_path: Path):
    ...
    return {
        ...
        "hardtail_stream_manifest_label": str(base_hardtail.sample_manifest_label),
        "reserve_stream_manifest_label": str(base_reserve.sample_manifest_label),
...
def main():
    ...
    payload = {
        ...
        "comparison_type": "tail_stream_selective_focal_reinforcement_readiness",
        ...
        "latest_tail_manifest_focal_reinforcement_deltas": {
        ...
        "hypothesis": (
            "Blanket manifest-tail focal reinforcement was effectively a no-op because both hardtail and reserve "
            "streams were scaled together. The next bounded question is whether the remaining FL-dominated camera "
            "gap is specific to the hardtail stream and can be closed by reinforcing only hardtail focal supervision."
        ),
        ...
        "reasoning": [
            "This family is not another blanket focal retry; it introduces a stream label and uses it to target only the hardtail cohort.",
            "The best current three-stream contract remains fixed, including hardtail max_depth_conf and reserve coverage.",
            "The new bounded question is whether hardtail-only focal reinforcement erases the last small FL camera tax while preserving the strongest depth gains.",
        ],
    }
    output_json.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lines = [
        "# Tail Stream-Selective Focal Reinforcement Readiness",
        "",
