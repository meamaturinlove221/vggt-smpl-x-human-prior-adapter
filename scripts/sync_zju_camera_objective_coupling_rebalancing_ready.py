...
DECISION_JSON = RESEARCH_ROOT / "execution_ready_promotion_decision.camera_objective_coupling_rebalancing.20260402.json"
...
def main() -> int:
    ...
    updated_task_plan.update(
        {
            ...
            "task_mode_focus": "camera_objective_coupling_rebalancing_execution_ready_pending_arm_cloud_off",
        }
    )
    ...
    updated_task_plan["problem_definition_progress"].update(
        {
            ...
            "newest_boundary_fact": "camera_objective_coupling_rebalancing is now packaged to execution-ready pending manual arm using the single flrelief090_tboost105 candidate on the stable lead.",
            "next_requirement": "Manually approve and arm the single execution-ready camera_objective_coupling_rebalancing candidate before any run.",
        }
    )
    updated_task_plan["active_tasks"] = [
        {
            "id": "manual_approval_to_arm_camera_objective_coupling_rebalancing",
            "status": "active",
            "details": "Manually approve and arm exactly one stablelead_global_fl_relief090_t_boost105 candidate on the current repo.",
        },
        {
            "id": "keep_cloud_off",
            "status": "active",
            "details": "Keep cloud_gate=false and launch_cloud_now=false while the execution-ready coupling candidate remains local-only.",
        },
    ]
    updated_task_plan["completed_this_round"] = upsert_by_key(
        list(updated_task_plan.get("completed_this_round", []) or []),
        {
            "id": "phase_40_camera_objective_coupling_execution_ready_packaged",
            "status": "completed",
            "details": "Packaged camera_objective_coupling_rebalancing to execution-ready pending manual arm with one flrelief090_tboost105 candidate.",
        },
        "id",
    )
    updated_task_plan["fastest_next_path"] = [
        "Keep guard and research idle/clean.",
        "Manually approve and arm exactly one camera_objective_coupling_rebalancing flrelief090_tboost105 candidate.",
    ...
    updated_task_plan["summary_conclusion"] = [
        "camera_objective_coupling_rebalancing is now the next execution-ready family.",
        "Exactly one stablelead_global_fl_relief090_t_boost105 candidate is pending explicit manual arm approval on the current repo.",
        "Cloud remains off and no second candidate may be opened.",
    ]
    updated_task_plan["current_state_notes"] = [
        "camera_objective_coupling_rebalancing is execution-ready pending manual arm approval.",
        "The coupling ticket keeps the stable lead fixed and touches only training/loss.py plus one config.",
    ...
    research_node["summary"] = {
        ...
        "current_review_packet": "output/zju_source_policy_research_loop/execution_ready_promotion_decision.camera_objective_coupling_rebalancing.20260402.json",
        "current_ready_candidate_config": config,
    }
    updated_watch["research"] = research_node
    updated_watch["watch_conclusion"] = (
        "guard is green and research remains idle; camera_objective_coupling_rebalancing is execution-ready pending explicit "
        "manual arm approval for one local flrelief090_tboost105 candidate, and cloud remains off"
    )
