...
BLUEPRINT_JSON = RESEARCH_ROOT / "family_blueprint.camera_objective_coupling_rebalancing.json"
FAMILY_PLAN_JSON = RESEARCH_ROOT / "candidate_patch_plan.camera_objective_coupling_rebalancing.json"
FAMILY_PLAN_MD = RESEARCH_ROOT / "candidate_patch_plan.camera_objective_coupling_rebalancing.md"
DRAFT_JSON = RESEARCH_ROOT / "next_manual_problem_draft.camera_objective_coupling_rebalancing.20260402.json"
DRAFT_MD = RESEARCH_ROOT / "next_manual_problem_draft.camera_objective_coupling_rebalancing.20260402.md"
...
def main() -> int:
    ...
    if family != "camera_objective_coupling_rebalancing":
        raise RuntimeError("candidate_verdict family must be camera_objective_coupling_rebalancing for post-verdict sync.")
    ...
    next_requirement = (
        ...
        "If another forward move is needed, it must come from a higher-level fresh manual problem rather than another camera sub-object counterbalance reopen."
    )
    ...
    updated_blueprint.update(
        {
            ...
            "why_now": "The single execution-ready FL/T coupling candidate has now been executed and closed with a dead_same_day short-gate verdict.",
            "latest_verdict": {
                "status": status,
                "reason": reason,
    ...
    family_readout["camera_objective_coupling_rebalancing"] = {
    ...
    updated_family_stop["latest_family_outcomes"]["camera_objective_coupling_rebalancing"] = {
    ...
    updated_research_status.update(
        {
            "checked_at": checked_at,
            "reason": (
                "No approved_problem.json is present; the single camera_objective_coupling_rebalancing candidate has already been "
                "executed and closed as dead_same_day, so research remains idle in guard."
            ),
            "current_cloud_blocker": (
                "camera_objective_coupling_rebalancing closed as dead_same_day at short_gate_10x5, and cloud must remain off. "
                "Any later forward move requires a fresh higher-level manual problem and a separate explicit downstream decision."
            ),
            "allowed_families": [],
            ...
            "preferred_first_candidate_execution_note": "Do not auto-arm a next ticket. Define a fresh higher-level manual problem first.",
    ...
    updated_task_plan.update(
        {
            ...
            "task_mode_focus": "camera_objective_coupling_rebalancing_dead_same_day_cloud_off",
        }
    )
    ...
    updated_task_plan["problem_definition_progress"].update(
        {
            "status": "camera_objective_coupling_rebalancing_dead_same_day",
            "newest_boundary_fact": "The single execution-ready FL/T coupling candidate still failed the multi-metric stable-lead rule because camera improved only marginally while T, conf_depth, and reg_depth did not improve, so the family is closed for this round.",
            "next_requirement": "Hold in IDLE_GUARD and wait for a fresh higher-level manual problem only if later needed.",
        }
    )
    updated_task_plan["completed_this_round"] = upsert_by_key(
        list(updated_task_plan.get("completed_this_round", []) or []),
        {
            "id": "phase_41_camera_objective_coupling_execution_closed_dead_same_day",
            "status": "completed",
            "details": "Ran the single execution-ready flrelief090_tboost105 candidate and closed camera_objective_coupling_rebalancing as dead_same_day at short_gate_10x5.",
        },
        "id",
    )
    updated_task_plan["completed_this_round"] = upsert_by_key(
        list(updated_task_plan.get("completed_this_round", []) or []),
        {
            "id": "phase_42_camera_objective_coupling_post_verdict_sync",
            "status": "completed",
            "details": "Synced coupling family/task/watch truth to the dead_same_day post-verdict state and kept cloud off.",
        },
        "id",
    )
    updated_task_plan["active_tasks"] = []
    blocked = list(updated_task_plan.get("blocked_tasks", []) or [])
    blocked = upsert_by_key(
        blocked,
        {
            "id": "rearm_camera_objective_coupling_rebalancing",
            "status": "blocked_by_formal_dead_same_day_verdict",
            "details": "The first camera_objective_coupling_rebalancing ticket already spent the current family budget and may not be re-armed this round.",
        },
        "id",
    )
    ...
    updated_task_plan["fastest_next_path"] = [
        ...
        "Do not arm a second camera_objective_coupling_rebalancing ticket.",
        "Keep cloud off.",
        "If another move is needed later, define a fresh higher-level manual problem first.",
    ]
    updated_task_plan["summary_conclusion"] = [
        "camera_objective_coupling_rebalancing / stablelead_global_fl_relief090_t_boost105 finished with a dead_same_day verdict at short_gate_10x5.",
        "Joint FL/T coupling did not break the plateau: camera improved only marginally while T, conf_depth, and reg_depth did not improve.",
        "Research is back in IDLE_GUARD, no approved problem remains, and cloud stays off.",
    ]
    updated_task_plan["current_state_notes"] = [
        "The single execution-ready camera_objective_coupling_rebalancing candidate has been executed and closed for this round.",
    ...
    updated_watch["watch_conclusion"] = (
        "guard is green and research remains idle; camera_objective_coupling_rebalancing closed as dead_same_day at short_gate_10x5, "
    ...
    print(
        json.dumps(
            {
                "candidate_verdict": "output/zju_source_policy_research_loop/candidate_verdict.json",
                ...
                "family_stop_reason": "output/zju_source_policy_research_loop/family_stop_reason.json",
                "family_blueprint": "output/zju_source_policy_research_loop/family_blueprint.camera_objective_coupling_rebalancing.json",
                "family_candidate_patch_plan": "output/zju_source_policy_research_loop/candidate_patch_plan.camera_objective_coupling_rebalancing.json",
