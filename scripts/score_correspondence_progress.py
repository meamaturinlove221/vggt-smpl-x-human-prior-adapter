...
def smoke_accept(row: dict) -> bool:
    return (
        ...
        and
        row["delta_fg_connected_components"] <= -1
        ...
        and row["fg_visible_rgb_coverage_ratio"] >= 0.78
        and row["fg_retained_mass_ratio"] >= 0.80
...
def hero_accept(compare: dict) -> bool:
    return (
        ...
        and compare["mean_fg_visible_rgb_coverage_ratio"] >= 0.80
        and compare["mean_fg_retained_mass_ratio"] >= 0.82
...
def local20_accept(compare: dict) -> bool:
    return (
        ...
        and compare["mean_delta_masked_ssim"] >= 0.0
        and compare["mean_fg_visible_rgb_coverage_ratio"] >= 0.80
        and compare["mean_fg_retained_mass_ratio"] >= 0.84
        and compare["mean_largest_fg_visible_component_ratio"] >= 0.60
        and compare["mean_human_erasure_penalty"] <= 0.035
        and compare["mean_delta_off_body_support_ratio"] <= 0.0
        and compare["mean_delta_bg_bottom_support_ratio"] <= 0.0
    )
...
def classify_failure(compare: dict) -> str:
    ...
    if (
        compare["mean_human_erasure_penalty"] > 0.08
        or compare["mean_fg_visible_rgb_coverage_ratio"] < 0.75
        or compare["mean_fg_retained_mass_ratio"] < 0.80
        or compare["mean_delta_fg_bbox_cover_ratio"] < -0.10
    ):
        return "erasure_win"
    no_move = (
...
def progress_key(compare: dict) -> tuple:
    return (
        ...
        -compare["mean_fg_visible_rgb_coverage_ratio"],
...
def effective_progress(compare: dict, best_compare: dict | None) -> bool:
    ...
    if compare["mean_delta_bg_bottom_support_ratio"] < best_compare["mean_delta_bg_bottom_support_ratio"]:
        better_count += 1
    if compare["mean_fg_visible_rgb_coverage_ratio"] > best_compare.get("mean_fg_visible_rgb_coverage_ratio", best_compare["mean_fg_rehydrated_coverage_ratio"]):
        better_count += 1
    if compare["mean_largest_fg_visible_component_ratio"] > best_compare.get("mean_largest_fg_visible_component_ratio", 0.0):
        better_count += 1
    if compare["mean_fg_retained_mass_ratio"] > best_compare["mean_fg_retained_mass_ratio"]:
        better_count += 1
    if compare["mean_human_erasure_penalty"] < best_compare["mean_human_erasure_penalty"]:
        better_count += 1
    return (
        better_count >= 4
        and compare["mean_fg_visible_rgb_coverage_ratio"] >= max(0.70, best_compare.get("mean_fg_visible_rgb_coverage_ratio", best_compare["mean_fg_rehydrated_coverage_ratio"]) - 0.02)
        and compare["mean_fg_retained_mass_ratio"] >= max(0.75, best_compare["mean_fg_retained_mass_ratio"] - 0.02)
    )
