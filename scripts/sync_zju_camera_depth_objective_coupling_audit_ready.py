...
DECISION_JSON = RESEARCH_ROOT / "execution_ready_promotion_decision.camera_depth_objective_coupling_audit.20260403.json"
...
def main() -> int:
    ...
    updated_task_plan.update(
        {
            ...
            "task_mode_focus": "camera_depth_objective_coupling_audit_execution_ready_pending_arm_cloud_off",
        }
    )
    ...
    updated_task_plan["problem_definition_progress"].update(
        {
            ...
            "newest_boundary_fact": "camera_depth_objective_coupling_audit is now packaged to execution-ready pending manual arm using the single cameraweight095_depthhold100 candidate on the stable lead.",
            "next_requirement": "Manually approve and arm the single execution-ready camera_depth_objective_coupling_audit candidate before any run.",
        }
    )
    updated_task_plan["active_tasks"] = [
        {
            "id": "manual_approval_to_arm_camera_depth_objective_coupling_audit",
            "status": "active",
            "details": "Manually approve and arm exactly one stablelead_global_cameraweight095_depthhold100 candidate on the current repo.",
        },
        {
            "id": "keep_cloud_off",
            "status": "active",
            "details": "Keep cloud_gate=false and launch_cloud_now=false while the execution-ready camera-depth audit candidate remains local-only.",
        },
    ]
    updated_task_plan["completed_this_round"] = upsert_by_key(
        list(updated_task_plan.get("completed_this_round", []) or []),
        {
            "id": "phase_43_camera_depth_objective_coupling_audit_execution_ready_packaged",
            "status": "completed",
            "details": "Packaged camera_depth_objective_coupling_audit to execution-ready pending manual arm with one cameraweight095_depthhold100 candidate.",
        },
        "id",
    )
    updated_task_plan["fastest_next_path"] = [
        "Keep guard and research idle/clean.",
        "Manually approve and arm exactly one camera_depth_objective_coupling_audit cameraweight095_depthhold100 candidate.",
    ...
    updated_task_plan["summary_conclusion"] = [
        "camera_depth_objective_coupling_audit is now the next execution-ready family.",
        "Exactly one stablelead_global_cameraweight095_depthhold100 candidate is pending explicit manual arm approval on the current repo.",
        "Cloud remains off and no second candidate may be opened.",
    ]
    updated_task_plan["current_state_notes"] = [
        "camera_depth_objective_coupling_audit is execution-ready pending manual arm approval.",
        "The first ticket keeps the stable lead fixed and changes only global camera weight while depth stays unchanged.",
    ...
    research_node["summary"] = {
        ...
        "current_review_packet": "output/zju_source_policy_research_loop/execution_ready_promotion_decision.camera_depth_objective_coupling_audit.20260403.json",
        "current_ready_candidate_config": config,
    }
    updated_watch["research"] = research_node
    updated_watch["watch_conclusion"] = (
        "guard is green and research remains idle; camera_depth_objective_coupling_audit is execution-ready pending explicit "
        "manual arm approval for one local cameraweight095_depthhold100 candidate, and cloud remains off"
    )
