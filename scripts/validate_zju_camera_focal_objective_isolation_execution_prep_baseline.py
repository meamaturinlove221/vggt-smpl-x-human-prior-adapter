...
VALIDATION_MD = OUTPUT_ROOT / "execution_prep_baseline_validation.camera_focal_objective_isolation.20260331.md"
...
def validate_signature_and_return_contract(loss_module, gt_pose):
    ...
    if "loss_fl_isolation_scale" not in sig.parameters:
        raise AssertionError("loss_fl_isolation_scale missing from compute_camera_loss signature")
    if sig.parameters["loss_fl_isolation_scale"].default != 1.0:
        raise AssertionError("loss_fl_isolation_scale default is not identity")
    if not any(param.kind == inspect.Parameter.VAR_KEYWORD for param in sig.parameters.values()):
        raise AssertionError("compute_camera_loss no longer accepts **kwargs")

    batch = make_batch(batch_size=2)
    pred_pose = make_predicted_pose(
        gt_pose,
        translation_offsets=[0.9, 0.3],
        rotation_offsets=[0.4, 0.2],
        focal_offsets=[0.2, 0.1],
    )
    ...
    return {
        ...
        "status": "pass",
        "details": {
            "loss_fl_isolation_scale_default": sig.parameters["loss_fl_isolation_scale"].default,
...
def validate_scale_only_affects_total_camera(loss_module, gt_pose):
    ...
    pred_pose = make_predicted_pose(
        gt_pose,
        translation_offsets=[0.7, 0.4],
        rotation_offsets=[0.5, 0.1],
        focal_offsets=[0.3, 0.05],
    )
    ...
    scaled = loss_module.compute_camera_loss(
        ...
        weight_rot=1.0,
        weight_focal=0.5,
        loss_fl_isolation_scale=1.75,
    )
    assert_close(base["loss_T"], scaled["loss_T"], message="loss_T changed under isolation scale")
    assert_close(base["loss_R"], scaled["loss_R"], message="loss_R changed under isolation scale")
    assert_close(base["loss_FL"], scaled["loss_FL"], message="loss_FL return changed under isolation scale")
    expected_scaled = base["loss_T"] + base["loss_R"] + base["loss_FL"] * 1.75 * 0.5
    ...
    return {
        ...
        "details": {
            ...
            "returned_loss_FL": float(base["loss_FL"]),
        },
    }
...
def validate_multistage_manual_accumulation(loss_module, gt_pose):
    batch = make_batch(batch_size=2)
    stage1 = make_predicted_pose(
        gt_pose,
        translation_offsets=[0.3, 0.1],
        rotation_offsets=[0.2, 0.05],
        focal_offsets=[0.4, 0.1],
    )
    stage2 = make_predicted_pose(
        gt_pose,
        translation_offsets=[0.9, 0.3],
        rotation_offsets=[0.5, 0.2],
        focal_offsets=[0.25, 0.15],
    )
    gamma = 0.6
    result = loss_module.compute_camera_loss(
    ...
    for idx, pred_stage in enumerate([stage1, stage2]):
        ...
        loss_t, loss_r, loss_fl = loss_module.camera_loss_single(
            pred_stage[valid_frame_mask].clone(),
            gt_pose_valid,
            loss_type="l1",
        )
    ...
    avg_r = total_r / 2
    avg_fl = total_fl / 2
    expected_camera = avg_t * 1.2 + avg_r * 0.8 + avg_fl * 0.5
...
def validate_manifest_and_train_only_gate(loss_module, gt_pose):
    batch_train = make_batch(batch_size=2)
    batch_train["selection_sample_manifest_applied"] = torch.tensor([1, 0], dtype=torch.int64)
    batch_train["selection_sample_manifest_label"] = ["focus", "other"]
    batch_train["_loss_phase"] = "train"
    batch_train["train_progress"] = 0.8

    batch_val = dict(batch_train)
    batch_val["_loss_phase"] = "val"

    pred_pose = make_predicted_pose(
        gt_pose,
        translation_offsets=[0.2, 0.2],
        rotation_offsets=[0.1, 0.1],
        focal_offsets=[0.8, 0.05],
    )
    common_kwargs = {
        "weight_trans": 1.0,
        "weight_rot": 1.0,
        ...
        "sample_manifest_applied_focal_scale": 1.5,
        "sample_manifest_label_focal_scales": {"focus": 1.0, "other": 1.0},
        "sample_manifest_applied_focal_scale_stage2": 2.0,
        "sample_manifest_label_focal_scales_stage2": {"focus": 2.5},
        "sample_manifest_schedule_start": 0.5,
        "sample_manifest_schedule_end": 0.9,
        "sample_manifest_applied_train_only": True,
    }
    baseline_train = loss_module.compute_camera_loss(
        {"pose_enc_list": [pred_pose]},
        batch_train,
        weight_trans=1.0,
        weight_rot=1.0,
    ...
    manifest_train = loss_module.compute_camera_loss(
        {"pose_enc_list": [pred_pose]},
        batch_train,
        **common_kwargs,
    )
    baseline_val = loss_module.compute_camera_loss(
        {"pose_enc_list": [pred_pose]},
        batch_val,
        weight_trans=1.0,
        weight_rot=1.0,
    ...
    manifest_val = loss_module.compute_camera_loss(
        {"pose_enc_list": [pred_pose]},
        batch_val,
        **common_kwargs,
    )

    if torch.allclose(manifest_train["loss_camera"], baseline_train["loss_camera"], atol=1e-6, rtol=1e-6):
        raise AssertionError("manifest train path did not affect loss_camera as expected")
    assert_close(manifest_val["loss_camera"], baseline_val["loss_camera"], message="train-only manifest gating failed on val path")
    for key in ("loss_camera", "loss_T", "loss_R", "loss_FL"):
        if not torch.isfinite(manifest_train[key]):
            raise AssertionError(f"non-finite {key} under manifest train path")
        if not torch.isfinite(manifest_val[key]):
            raise AssertionError(f"non-finite {key} under manifest val path")
    return {
        "name": "manifest_and_train_only_gate",
        "status": "pass",
        "details": {
            "baseline_train_loss_camera": float(baseline_train["loss_camera"]),
            "manifest_train_loss_camera": float(manifest_train["loss_camera"]),
            "baseline_val_loss_camera": float(baseline_val["loss_camera"]),
            "manifest_val_loss_camera": float(manifest_val["loss_camera"]),
        },
    }
...
def render_md(payload: dict) -> str:
    lines = [
        "# Execution-Prep Baseline Validation: Camera Focal Objective Isolation (2026-03-31)",
        "",
...
def main() -> int:
    ...
    validation_cases = [
        ...
        validate_manifest_and_train_only_gate(loss_module, gt_pose),
    ]
    ...
    payload = {
        ...
        "summary": (
            "The reviewed loss.py-only patch passed stronger local validation: the default formula is preserved, "
            "the isolation scale only changes total loss_camera aggregation, the multistage accumulation matches manual expectations, "
            "and the manifest/train-only path remains callable and gated correctly without training execution."
        ),
        "environment_note": {
        ...
        "next_requirement": "decide_validated_loss_py_patch_can_enter_execution_prep_baseline",
    }
