...
OUT_ROOT = OUTPUT / "V17300000000000000000_multishell_topology_decoder_training"
...
def build_batch(row: dict[str, str], max_points: int, device: torch.device) -> tuple[dict[str, torch.Tensor], dict[str, torch.Tensor], dict[str, np.ndarray]]:
    ...
    priority = repair.astype(np.float32) * 8.0 + shell_conf * 1.5 + weak_score * 0.6 + conf * 0.04
    ...
    smpl[:, cfg.part_count + 3] = shell_conf[idx]
    batch = {
        ...
        "weak_region": torch.from_numpy(np.maximum(repair[idx].astype(np.float32), weak_score[idx] * 0.25)[None]).to(device),
...
def hard_control_scores(case: str) -> dict[str, float]:
    out: dict[str, float] = {}
    for config in CONTROL_CONFIGS:
        ...
        out[config] = float(anti_billboard_metric_v2(np.asarray(pred["human_points"], dtype=np.float32), body)["anti_billboard_score_v2"])
    return out


def training_losses(out: dict[str, torch.Tensor], targets: dict[str, torch.Tensor], control_scores: dict[str, float]) -> dict[str, torch.Tensor]:
    ...
    side_l = ((out["left_shell"] - targets["side_shell_target"]) ** 2 * weak).mean()
    side_r = ((out["right_shell"] - targets["side_shell_target"]) ** 2 * weak).mean()
    ...
    fb_width = (out["front_shell"] - out["back_shell"]).norm(dim=-1)
    lr_width = (out["left_shell"] - out["right_shell"]).norm(dim=-1)
    weak_scalar = targets["weak"].squeeze(-1)
    min_width = 0.065 + 0.020 * weak_scalar
    shell_width_loss = (F.relu(min_width - fb_width) + F.relu(min_width * 0.70 - lr_width)).mean()
    # Keep shells part-continuous by discouraging large point-to-point jumps along
    # the selected anchor ordering. This is a cheap differentiable adjacency proxy.
    shell_stack = torch.stack([out["front_shell"], out["back_shell"], out["left_shell"], out["right_shell"]], dim=2)
    adjacency = (shell_stack[:, 1:] - shell_stack[:, :-1]).norm(dim=-1).mean()
    adjacency_loss = torch.relu(adjacency - 0.090)
    ...
    volume_loss = F.relu(0.035 - eig[0] / eig[-1].clamp_min(1e-8))
    strongest = max(control_scores.values()) if control_scores else 0.0
    adv_margin = torch.relu(torch.tensor(strongest + 0.10, device=pts.device) - (fb_width.mean() + lr_width.mean()) * 4.5)
    total = (
        front
        + back
        + 0.5 * (side_l + side_r)
...
def balanced_multishell_decode(out: dict[str, torch.Tensor], aux: dict[str, np.ndarray], targets: dict[str, torch.Tensor]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    base_points = aux["human_points"]
    base_rgb = aux["human_rgb"]
    body = aux["body_part_id"]
    idx = aux["idx"]
    repair_selected = aux["repair"][idx]
    ...
    # Replace weak/billboard anchors with multi-shell decoded points. Keep
    # high-confidence no-change zones untouched.
    selected_replace = repair_selected | (weak > 0.35)
    ...
    if len(merged_points) < len(base_points):
        ...
        priority = np.concatenate([np.zeros(int(keep.sum()), dtype=np.float32) + 0.5, np.repeat(aux["priority"][idx[selected_replace]], shells.shape[1])], axis=0)
...
def train_case(row: dict[str, str], steps: int, max_points: int, device: torch.device) -> dict[str, Any]:
    ...
    np.savez_compressed(
        ...
        multishell_decoded=np.array(True),
    )
...
def render_panel(points: np.ndarray, colors: np.ndarray, title: str, *, rot: np.ndarray | None = None) -> Image.Image:
    size = (390, 282)
    ...
    rgb = np.clip(colors.astype(np.float32) * (0.62 + 0.46 * cue[:, None]), 0, 255).astype(np.uint8)
    ...
    draw.text((8, 8), title[:68], fill=(10, 10, 10))
    return im
...
def cross_panel(points: np.ndarray, title: str) -> Image.Image:
    ...
    draw = ImageDraw.Draw(im)
    if len(points) < 8:
        draw.text((8, 8), title + " empty", fill=(150, 0, 0))
        return im
    ...
    q[:, 1] = 1.0 - q[:, 1]
    xy = np.clip(q * np.array([size[0] - 48, size[1] - 68]) + np.array([24, 44]), 0, [size[0] - 1, size[1] - 1]).astype(int)
    order_i = np.argsort(proj[:, 0])
    step = max(1, len(order_i) // 52000)
    for i in order_i[::step]:
        x, y = xy[i]
        im.putpixel((int(x), int(y)), (42, 68, 55))
        if 1 <= x < size[0] - 1 and 1 <= y < size[1] - 1:
            im.putpixel((int(x + 1), int(y)), (83, 110, 91))
    draw.text((8, 8), title[:68], fill=(10, 10, 10))
...
def compare(rows: list[dict[str, str]], manifest: list[dict[str, Any]], created_at: str) -> None:
    ...
    for row in rows:
        ...
        for config in CONTROL_CONFIGS:
            score = float(case_metrics[config]["anti_billboard_score_v2"])
            if score >= true_score * 0.96:
                failures.append({"case": case, "reason": "control_close_or_better_v2", "control": config, "true_score": true_score, "control_score": score})
    write_csv(REPORTS / "V17300000000000000000_seed_metrics.csv", metric_rows)
    first = rows[0]["case"]
    panels = []
    ...
    compose(panels, 3, BOARDS / "V17300000000000000000_multishell_training_board.png")
    turn = []
    ...
    compose(turn, 3, BOARDS / "V17300000000000000000_turntable_cross_section.png")
    decision = {
        "created_at": created_at,
        "status": "V17300_MULTISHELL_TOPOLOGY_DECODER_FAIL_CLOSED_CONTINUE" if failures else "V17300_MULTISHELL_PRECHECK_PASS_REQUIRES_MENTOR_GATE",
        "mentor_ready": False,
        "external_hard_block": False,
        "cases": [r["case"] for r in rows],
        "training_manifest_rows": len(manifest),
        "failures": failures,
        "board": str(BOARDS / "V17300000000000000000_multishell_training_board.png"),
        "turntable_cross_section": str(BOARDS / "V17300000000000000000_turntable_cross_section.png"),
        "face_detail_claim_allowed": False,
        "allowed_face_claim": ALLOWED_FACE_CLAIM,
        "summary": "V173 decodes multi-shell points into the final point budget instead of discarding shells after loss, but still must fail closed unless controls are beaten.",
    }
    write_json(REPORTS / "V17300000000000000000_training_decision.json", decision)
...
def main() -> int:
    ...
    steps = int(os.environ.get("V17300_STEPS", "40"))
    max_points = int(os.environ.get("V17300_MAX_POINTS", "4096"))
    manifest = [train_case(row, steps=steps, max_points=max_points, device=device) for row in rows]
    write_csv(REPORTS / "V17300000000000000000_training_manifest.csv", manifest)
    compare(rows, manifest, created_at)
    write_json(REPORTS / "V17300000000000000000_runtime_environment.json", {"created_at": created_at, **runtime, "steps": steps, "max_points": max_points})
    print(json.dumps({"created_at": created_at, "status": "V17300_DONE", "device": str(device), "mentor_ready": False}, indent=2))
    return 0
