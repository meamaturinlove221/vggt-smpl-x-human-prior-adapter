...
BOARDS = REPO / "boards"
OUT_ROOT = OUTPUT / "V20700000000000000000_proposal_conditioned_decoder_training"
PROPOSAL_ROOT = OUTPUT / "V20500000000000000000_learned_part_proposals"
TRUE_CONFIG = "proposal_conditioned_decoder_true"
...
from models.v207_proposal_conditioned_decoder import ProposalConditionedDecoder  # noqa: E402
...
def train_case(row: dict[str, str], steps: int, max_points: int, device: torch.device) -> dict[str, Any]:
    model = ProposalConditionedDecoder().to(device)
    ...
    for step in range(steps):
        ...
        target_occ = torch.relu(0.50 - (out["occupancy"] * target_sel * proposal_weight).sum() / (target_sel * proposal_weight).sum().clamp_min(1.0))
        outside_occ = (out["occupancy"] * non_preserve_sel * (1.0 - target_sel)).mean()
        connected_occ = torch.relu(0.32 - (out["occupancy"] * target_sel * torch.maximum(weak_sel, target["anchor_affinity"])).sum() / (target_sel * torch.maximum(weak_sel, target["anchor_affinity"])).sum().clamp_min(1.0))
...
def compare(rows: list[dict[str, str]], created_at: str) -> None:
    ...
    configs = [TRUE_CONFIG, V206_CONFIG, V204_CONFIG, V203_CONFIG, V202_CONFIG, V200_CONFIG, V197_CONFIG, V194_CONFIG, V192_CONFIG, V190_CONFIG, V187_CONFIG, V186_CONFIG, BASELINE_CONFIG, "same_topology_no_semantic", "shuffled_smpl_feature", "thickness_only_control"]
    for row in rows:
        case = row["case"]
        ...
        if true is None:
            failures.append({"case": case, "reason": "missing_proposal_conditioned_decoder_true"})
            continue
        ...
        for cfg in [V206_CONFIG, V204_CONFIG, V203_CONFIG, V202_CONFIG, V200_CONFIG, V197_CONFIG, V194_CONFIG, V192_CONFIG, V190_CONFIG, V187_CONFIG, V186_CONFIG, BASELINE_CONFIG, "same_topology_no_semantic", "shuffled_smpl_feature", "thickness_only_control"]:
    ...
    write_csv(REPORTS / "V20700000000000000000_proposal_conditioned_decoder_scores.csv", score_rows)
    ...
    if panels:
        compose(panels, 3, BOARDS / "V20700000000000000000_proposal_conditioned_decoder_board.png")
    if first and config_path(first, TRUE_CONFIG).exists():
        pts, rgb, _body = load_config(first, TRUE_CONFIG)
        turn = [
            render_panel(pts, rgb, f"{first} proposal-conditioned {label}", rot=rot)
            for label, rot in [
        ...
        compose(turn, 3, BOARDS / "V20700000000000000000_proposal_conditioned_decoder_turntable_cross_section.png")
    decision = {
        "created_at": created_at,
        "status": "V207_PROPOSAL_CONDITIONED_DECODER_FAIL_CLOSED_CONTINUE" if failures else "V207_PROPOSAL_CONDITIONED_DECODER_PRECHECK_PASS_REQUIRES_MENTOR_VISUAL",
        "mentor_ready": False,
        "external_hard_block": False,
        "failures": failures,
        "score_csv": str(REPORTS / "V20700000000000000000_proposal_conditioned_decoder_scores.csv"),
        "board": str(BOARDS / "V20700000000000000000_proposal_conditioned_decoder_board.png"),
        "turntable_cross_section": str(BOARDS / "V20700000000000000000_proposal_conditioned_decoder_turntable_cross_section.png"),
        "face_detail_claim_allowed": False,
        "allowed_face_claim": ALLOWED_FACE_CLAIM,
        "summary": "V207 injects V205 proposal scores into occupancy/residual decoding rather than only post-decode selection. It remains fail-closed unless full-scene mentor visuals and hard controls pass.",
    }
    write_json(REPORTS / "V20700000000000000000_proposal_conditioned_decoder_decision.json", decision)
...
def main() -> int:
    ...
    steps = int(os.environ.get("V20700_STEPS", "40"))
    max_points = int(os.environ.get("V20700_MAX_POINTS", "8192"))
    manifest = [train_case(row, steps=steps, max_points=max_points, device=device) for row in rows]
    write_csv(REPORTS / "V20700000000000000000_proposal_conditioned_decoder_training_manifest.csv", manifest)
    compare(rows, created_at)
    write_json(
        REPORTS / "V20700000000000000000_runtime_environment.json",
        {"created_at": created_at, **runtime, "steps": steps, "max_points": max_points},
    )
    print(json.dumps({"created_at": created_at, "status": "V207_DONE", "device": str(device), "mentor_ready": False}, indent=2))
    return 0
