...
BOARDS = REPO / "boards"
OUT_ROOT = OUTPUT / "V20200000000000000000_visible_nonregression_connected_infill"
TRUE_CONFIG = "visible_nonregression_connected_infill_true"
...
def train_case(row: dict[str, str], steps: int, max_points: int, device: torch.device) -> dict[str, Any]:
    ...
    for step in range(steps):
        ...
        connected_occ = torch.relu(0.46 - (out["occupancy"] * non_preserve_sel * torch.maximum(weak_sel, target["anchor_affinity"])).sum() / (non_preserve_sel * torch.maximum(weak_sel, target["anchor_affinity"])).sum().clamp_min(1.0))
        ...
        total = 0.58 * losses["total"] + 2.30 * preserve_residual + 0.90 * visible_rgb_lock + 0.58 * preserve_occ + 0.52 * connected_occ + 0.74 * floating
        total.backward()
    ...
    keep_base = preserve
    visible_dist = nearest_distance(infill_points, baseline_points[keep_base])
    weak_dist = nearest_distance(infill_points, baseline_points[~keep_base] if bool(np.any(~keep_base)) else baseline_points)
    # Only allow a small connected infill budget. Anything far from the locked
    # visible surface is the noisy-cloud failure seen in V194.
    infill_count = min(9000, max(5000, 60000 - int(keep_base.sum())))
    low = float(np.quantile(visible_dist, 0.08))
    high = float(np.quantile(visible_dist, 0.34))
    score = -np.abs(visible_dist - np.quantile(visible_dist, 0.20)) / max(np.quantile(visible_dist, 0.20), 1e-6) + 0.55 * np.exp(-weak_dist / max(float(np.quantile(weak_dist, 0.45)), 1e-6))
    connected_ids = np.flatnonzero((visible_dist >= low) & (visible_dist <= high))
    if len(connected_ids) > infill_count:
        connected_ids = connected_ids[np.argsort(-score[connected_ids])[:infill_count]]
    elif len(connected_ids) < infill_count:
        connected_ids = np.unique(np.concatenate([connected_ids, np.argsort(-score)[:infill_count]]))
    ...
    np.savez_compressed(
        ...
        connected_infill_distance_mean=np.array(float(np.mean(visible_dist[connected_ids])) if len(connected_ids) else 0.0),
        ...
        visible_nonregression_connected_infill=np.array(True),
    ...
    return {
        ...
        "connected_infill_distance_mean": float(np.mean(visible_dist[connected_ids])) if len(connected_ids) else 0.0,
        **preserve_info,
        **metric,
    }
...
def compare(rows: list[dict[str, str]], created_at: str) -> None:
    ...
    configs = [TRUE_CONFIG, V200_CONFIG, V197_CONFIG, V194_CONFIG, V192_CONFIG, V190_CONFIG, V187_CONFIG, V186_CONFIG, BASELINE_CONFIG, "same_topology_no_semantic", "shuffled_smpl_feature", "thickness_only_control"]
    for row in rows:
        case = row["case"]
        ...
        if true is None:
            failures.append({"case": case, "reason": "missing_visible_nonregression_connected_true"})
            continue
        ...
        for cfg in [V200_CONFIG, V197_CONFIG, V194_CONFIG, V192_CONFIG, V190_CONFIG, V187_CONFIG, V186_CONFIG, BASELINE_CONFIG, "same_topology_no_semantic", "shuffled_smpl_feature", "thickness_only_control"]:
    ...
    write_csv(REPORTS / "V20200000000000000000_visible_nonregression_connected_scores.csv", score_rows)
    ...
    if panels:
        compose(panels, 3, BOARDS / "V20200000000000000000_visible_nonregression_connected_board.png")
    if first and config_path(first, TRUE_CONFIG).exists():
        pts, rgb, _body = load_config(first, TRUE_CONFIG)
        turn = [
            render_panel(pts, rgb, f"{first} connected {label}", rot=rot)
            for label, rot in [
        ...
        turn.append(cross_panel(pts, f"{first} connected cross-section"))
        compose(turn, 3, BOARDS / "V20200000000000000000_visible_nonregression_connected_turntable_cross_section.png")
    decision = {
        "created_at": created_at,
        "status": "V20200_VISIBLE_NONREGRESSION_CONNECTED_FAIL_CLOSED_CONTINUE" if failures else "V20200_VISIBLE_NONREGRESSION_CONNECTED_PRECHECK_PASS_REQUIRES_MENTOR_VISUAL",
        "mentor_ready": False,
        "external_hard_block": False,
        "failures": failures,
        "score_csv": str(REPORTS / "V20200000000000000000_visible_nonregression_connected_scores.csv"),
        "board": str(BOARDS / "V20200000000000000000_visible_nonregression_connected_board.png"),
        "turntable_cross_section": str(BOARDS / "V20200000000000000000_visible_nonregression_connected_turntable_cross_section.png"),
        "face_detail_claim_allowed": False,
        "allowed_face_claim": ALLOWED_FACE_CLAIM,
        "summary": "V202 trains visible-surface non-regression and connected weak-region infill. It remains fail-closed unless full-scene mentor visuals and hard controls pass.",
    }
    write_json(REPORTS / "V20200000000000000000_visible_nonregression_connected_decision.json", decision)
...
def main() -> int:
    ...
    steps = int(os.environ.get("V20200_STEPS", "40"))
    max_points = int(os.environ.get("V20200_MAX_POINTS", "8192"))
    manifest = [train_case(row, steps=steps, max_points=max_points, device=device) for row in rows]
    write_csv(REPORTS / "V20200000000000000000_visible_nonregression_connected_training_manifest.csv", manifest)
    compare(rows, created_at)
    write_json(
        REPORTS / "V20200000000000000000_runtime_environment.json",
        {"created_at": created_at, **runtime, "steps": steps, "max_points": max_points},
    )
    print(json.dumps({"created_at": created_at, "status": "V20200_DONE", "device": str(device), "mentor_ready": False}, indent=2))
    return 0
