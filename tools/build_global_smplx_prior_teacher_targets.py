...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build diagnostic SMPL-X teacher targets with one global VGGT-world Sim(3). "
            "Unlike per-view prior alignment, this keeps all views in a single fused coordinate frame."
        )
    )
    parser.add_argument("--scene-dir", required=True, type=Path)
    parser.add_argument("--base-predictions", required=True, type=Path)
    parser.add_argument("--output-dir", required=True, type=Path)
    parser.add_argument("--align-roi", choices=("full", "head", "face", "head_face"), default="full")
    ...
    parser.add_argument("--max-correspondences", type=int, default=180000)
...
def camera_to_world(points_camera: np.ndarray, extrinsic: np.ndarray) -> np.ndarray:
    ...
    return np.einsum("vij,vhwj->vhwi", rotation, points_camera.astype(np.float64)) + translation[:, None, None, :]


def estimate_umeyama(source: np.ndarray, target: np.ndarray) -> tuple[float, np.ndarray, np.ndarray]:
    if source.shape[0] < 32:
        raise RuntimeError(f"Need at least 32 correspondences, got {source.shape[0]}")
...
def robust_similarity(
    ...
    *,
    ...
    residual = np.linalg.norm(pred - target, axis=1)
    summary = {
        "input_correspondences": original,
...
def main() -> int:
    ...
    if prior_maps.shape[0] != view_count or prior_maps.shape[2:] != (height, width):
        raise ValueError(f"prior maps shape {prior_maps.shape} does not match base {base_world.shape}")
    ...
    smplx_pseudo_world = camera_to_world(smplx_cam, extrinsic)

    source_parts: list[np.ndarray] = []
    target_parts: list[np.ndarray] = []
    roi_masks = np.zeros(base_world.shape[:3], dtype=bool)
    apply_masks = np.zeros(base_world.shape[:3], dtype=bool)
    ...
    for view_idx in range(view_count):
        ...
        align_mask = (
            ...
            & np.isfinite(smplx_pseudo_world[view_idx]).all(axis=-1)
            & (base_conf[view_idx] >= threshold)
        )
        if align_mask.any():
            source_parts.append(smplx_pseudo_world[view_idx][align_mask])
            target_parts.append(base_world[view_idx][align_mask])
        roi_masks[view_idx] = align_roi
        apply_masks[view_idx] = visible[view_idx] & apply_roi
        per_view.append(
            {
                "view_index": int(view_idx),
                "align_roi_pixels": int(align_roi.sum()),
                "apply_roi_pixels": int(apply_roi.sum()),
                "visible_apply_pixels": int(apply_masks[view_idx].sum()),
                "base_conf_threshold": float(threshold),
                "align_correspondences": int(align_mask.sum()),
            }
        )
    if not source_parts:
        raise RuntimeError("No correspondences available for global SMPL-X prior alignment.")
    ...
    teacher_world = apply_similarity(smplx_pseudo_world, scale, rotation, translation)
    teacher_world[~apply_masks] = 0.0

    # Camera normals are converted through VGGT camera-to-world and global
    # rotation. This is diagnostic metadata; fusion uses the point/depth maps.
    cam_to_world = closed_form_inverse_se3(extrinsic)
    pseudo_world_normals = np.einsum("vij,vhwj->vhwi", cam_to_world[:, :3, :3], smplx_normals.astype(np.float64))
    teacher_normals = normalize(pseudo_world_normals @ rotation.T).astype(np.float32)
    teacher_normals[~apply_masks] = 0.0

    distance = np.linalg.norm(teacher_world - base_world, axis=-1)
    valid_distance = apply_masks & np.isfinite(distance)
    np.savez_compressed(
        output_dir / "teacher_targets.npz",
        world_points=teacher_world.astype(np.float32),
        teacher_mask=apply_masks.astype(bool),
        teacher_normals=teacher_normals.astype(np.float32),
        roi_mask=roi_masks.astype(bool),
    )
    summary = {
        "task": "global_smplx_prior_teacher_targets",
        "truthful_status": "coarse_smplx_global_alignment_diagnostic_not_final_face_teacher",
        ...
        "distance_to_base_percentiles_on_teacher_mask": [
            float(v) for v in np.percentile(distance[valid_distance], [0, 25, 50, 75, 90, 95, 99])
        ]
        if valid_distance.any()
        else [],
        "notes": [
            "One global transform is used for all views to avoid the fragmented per-view SMPL-X scaffold failure.",
            "SMPL-X remains a coarse topology prior. Template-looking face, hair, or clothing is still a visual failure.",
        ],
    }
    (output_dir / "global_smplx_teacher_summary.json").write_text(
