...
BOARDS = REPO / "boards"
OUT_ROOT = OUTPUT / "V20600000000000000000_learned_part_proposal_student"
PROPOSAL_ROOT = OUTPUT / "V20500000000000000000_learned_part_proposals"
TRUE_CONFIG = "learned_part_proposal_student_true"
...
def train_case(row: dict[str, str], steps: int, max_points: int, device: torch.device) -> dict[str, Any]:
    ...
    np.savez_compressed(
        ...
        learned_part_proposal_student=np.array(True),
...
def compare(rows: list[dict[str, str]], created_at: str) -> None:
    ...
    configs = [TRUE_CONFIG, V204_CONFIG, V203_CONFIG, V202_CONFIG, V200_CONFIG, V197_CONFIG, V194_CONFIG, V192_CONFIG, V190_CONFIG, V187_CONFIG, V186_CONFIG, BASELINE_CONFIG, "same_topology_no_semantic", "shuffled_smpl_feature", "thickness_only_control"]
    for row in rows:
        case = row["case"]
        ...
        if true is None:
            failures.append({"case": case, "reason": "missing_learned_part_proposal_student_true"})
            continue
        ...
        for cfg in [V204_CONFIG, V203_CONFIG, V202_CONFIG, V200_CONFIG, V197_CONFIG, V194_CONFIG, V192_CONFIG, V190_CONFIG, V187_CONFIG, V186_CONFIG, BASELINE_CONFIG, "same_topology_no_semantic", "shuffled_smpl_feature", "thickness_only_control"]:
    ...
    write_csv(REPORTS / "V20600000000000000000_learned_part_proposal_student_scores.csv", score_rows)
    ...
    if panels:
        compose(panels, 3, BOARDS / "V20600000000000000000_learned_part_proposal_student_board.png")
    if first and config_path(first, TRUE_CONFIG).exists():
        pts, rgb, _body = load_config(first, TRUE_CONFIG)
        turn = [
            render_panel(pts, rgb, f"{first} learned-proposal {label}", rot=rot)
            for label, rot in [
        ...
        compose(turn, 3, BOARDS / "V20600000000000000000_learned_part_proposal_student_turntable_cross_section.png")
    decision = {
        "created_at": created_at,
        "status": "V206_LEARNED_PART_PROPOSAL_STUDENT_FAIL_CLOSED_CONTINUE" if failures else "V206_LEARNED_PART_PROPOSAL_STUDENT_PRECHECK_PASS_REQUIRES_MENTOR_VISUAL",
        "mentor_ready": False,
        "external_hard_block": False,
        "failures": failures,
        "score_csv": str(REPORTS / "V20600000000000000000_learned_part_proposal_student_scores.csv"),
        "board": str(BOARDS / "V20600000000000000000_learned_part_proposal_student_board.png"),
        "turntable_cross_section": str(BOARDS / "V20600000000000000000_learned_part_proposal_student_turntable_cross_section.png"),
        "face_detail_claim_allowed": False,
        "allowed_face_claim": ALLOWED_FACE_CLAIM,
        "summary": "V206 trains a conservative student with V205 learned proposal scores as soft edit anchors. It remains fail-closed unless full-scene mentor visuals and hard controls pass.",
    }
    write_json(REPORTS / "V20600000000000000000_learned_part_proposal_student_decision.json", decision)
...
def main() -> int:
    ...
    steps = int(os.environ.get("V20600_STEPS", "40"))
    max_points = int(os.environ.get("V20600_MAX_POINTS", "8192"))
    manifest = [train_case(row, steps=steps, max_points=max_points, device=device) for row in rows]
    write_csv(REPORTS / "V20600000000000000000_learned_part_proposal_student_training_manifest.csv", manifest)
    compare(rows, created_at)
    write_json(
        REPORTS / "V20600000000000000000_runtime_environment.json",
        {"created_at": created_at, **runtime, "steps": steps, "max_points": max_points},
    )
    print(json.dumps({"created_at": created_at, "status": "V206_DONE", "device": str(device), "mentor_ready": False}, indent=2))
    return 0
