...
OUT_ROOT = OUTPUT / "V18500000000000000000_canonical_surfel_graph_training"
...
TRUE_CONFIG = "canonical_surfel_graph_true"
...
def graph_losses(out: dict[str, torch.Tensor], target: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    ...
    total = occ_loss + 0.45 * vis_loss + 0.24 * part_loss + 0.18 * exclusion_loss + 0.32 * thickness_loss + 0.18 * residual_loss + 0.36 * shell_loss
    return {
        "total": total,
        "occupancy": occ_loss,
        "visibility": vis_loss,
...
def decode(out: dict[str, torch.Tensor], aux: dict[str, np.ndarray], target: dict[str, torch.Tensor]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    rgb_sel = aux["rgb"][idx]
    keep = occ > max(0.34, float(np.quantile(occ, 0.38)))
...
def train_case(row: dict[str, str], steps: int, max_points: int, device: torch.device) -> dict[str, Any]:
    ...
    np.savez_compressed(
        ...
        canonical_surfel_graph_occupancy=np.array(True),
    )
...
def compare(rows: list[dict[str, str]], created_at: str) -> None:
    ...
    configs = [TRUE_CONFIG, V173_CONFIG, V183_CONFIG, *CONTROL_CONFIGS]
    for row in rows:
        case = row["case"]
        ...
        for cfg in [V173_CONFIG, V183_CONFIG, "real_vggt_baseline_only", "same_topology_no_semantic", "shuffled_smpl_feature", "thickness_only_control"]:
    ...
    write_csv(REPORTS / "V18500000000000000000_canonical_surfel_scores.csv", score_rows)
    first = rows[0]["case"] if rows else ""
    panels: list[Image.Image] = []
    for cfg in [TRUE_CONFIG, V173_CONFIG, V183_CONFIG, "real_vggt_baseline_only", "same_topology_no_semantic", "shuffled_smpl_feature"]:
        path = config_path(first, cfg)
        if path.exists():
            pts, rgb, _body = load_config(first, cfg)
            panels.append(render_panel(pts, rgb, f"{first} {cfg.replace('_', ' ')}"))
    if panels:
        compose(panels, 3, BOARDS / "V18500000000000000000_canonical_surfel_board.png")
    if first and config_path(first, TRUE_CONFIG).exists():
        ...
        compose(turn, 3, BOARDS / "V18500000000000000000_canonical_surfel_turntable_cross_section.png")
    decision = {
        "created_at": created_at,
        "status": "V18500_CANONICAL_SURFEL_GRAPH_TRAINING_FAIL_CLOSED_CONTINUE" if failures else "V18500_CANONICAL_SURFEL_PRECHECK_PASS_REQUIRES_MENTOR_VISUAL",
        "mentor_ready": False,
        "external_hard_block": False,
        "cases": [r["case"] for r in rows],
        "failures": failures,
        "score_csv": str(REPORTS / "V18500000000000000000_canonical_surfel_scores.csv"),
        "board": str(BOARDS / "V18500000000000000000_canonical_surfel_board.png"),
        "turntable_cross_section": str(BOARDS / "V18500000000000000000_canonical_surfel_turntable_cross_section.png"),
        "face_detail_claim_allowed": False,
        "allowed_face_claim": ALLOWED_FACE_CLAIM,
        "summary": "V185 trains a canonical SMPL-X surfel/graph occupancy student. It remains fail-closed unless full-scene mentor visuals and hard controls pass.",
    }
    write_json(REPORTS / "V18500000000000000000_training_decision.json", decision)
...
def main() -> int:
    ...
    steps = int(os.environ.get("V18500_STEPS", "40"))
    max_points = int(os.environ.get("V18500_MAX_POINTS", "8192"))
    manifest = [train_case(row, steps=steps, max_points=max_points, device=device) for row in rows]
    write_csv(REPORTS / "V18500000000000000000_training_manifest.csv", manifest)
    compare(rows, created_at)
    write_json(
        REPORTS / "V18500000000000000000_runtime_environment.json",
        {"created_at": created_at, **runtime, "steps": steps, "max_points": max_points},
    )
    print(json.dumps({"created_at": created_at, "status": "V18500_DONE", "device": str(device), "mentor_ready": False}, indent=2))
    return 0
