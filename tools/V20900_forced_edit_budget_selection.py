...
BOARDS = REPO / "boards"
OUT_ROOT = OUTPUT / "V20900000000000000000_forced_edit_budget_selection"
PROPOSAL_ROOT = OUTPUT / "V20500000000000000000_learned_part_proposals"
TRUE_CONFIG = "forced_edit_budget_selection_true"
...
def compare(rows: list[dict[str, str]], created_at: str) -> None:
    ...
    configs = [TRUE_CONFIG, V208_CONFIG, V207_CONFIG, V206_CONFIG, V204_CONFIG, V203_CONFIG, V202_CONFIG, V200_CONFIG, V197_CONFIG, V194_CONFIG, V192_CONFIG, V190_CONFIG, V187_CONFIG, V186_CONFIG, BASELINE_CONFIG, "same_topology_no_semantic", "shuffled_smpl_feature", "thickness_only_control"]
    for row in rows:
        case = row["case"]
        ...
        if true is None:
            failures.append({"case": case, "reason": "missing_forced_edit_budget_selection_true"})
            continue
        ...
        for cfg in [V208_CONFIG, V207_CONFIG, V206_CONFIG, V204_CONFIG, V203_CONFIG, V202_CONFIG, V200_CONFIG, V197_CONFIG, V194_CONFIG, V192_CONFIG, V190_CONFIG, V187_CONFIG, V186_CONFIG, BASELINE_CONFIG, "same_topology_no_semantic", "shuffled_smpl_feature", "thickness_only_control"]:
    ...
    write_csv(REPORTS / "V20900000000000000000_forced_edit_budget_selection_scores.csv", score_rows)
    ...
    if panels:
        compose(panels, 3, BOARDS / "V20900000000000000000_forced_edit_budget_selection_board.png")
    if first and config_path(first, TRUE_CONFIG).exists():
        pts, rgb, _body = load_config(first, TRUE_CONFIG)
        turn = [
            render_panel(pts, rgb, f"{first} forced edit-budget {label}", rot=rot)
            for label, rot in [
        ...
        compose(turn, 3, BOARDS / "V20900000000000000000_forced_edit_budget_selection_turntable_cross_section.png")
    decision = {
        "created_at": created_at,
        "status": "V209_FORCED_EDIT_BUDGET_SELECTION_FAIL_CLOSED_CONTINUE" if failures else "V209_FORCED_EDIT_BUDGET_SELECTION_PRECHECK_PASS_REQUIRES_MENTOR_VISUAL",
        "mentor_ready": False,
        "external_hard_block": False,
        "failures": failures,
        "score_csv": str(REPORTS / "V20900000000000000000_forced_edit_budget_selection_scores.csv"),
        "board": str(BOARDS / "V20900000000000000000_forced_edit_budget_selection_board.png"),
        "turntable_cross_section": str(BOARDS / "V20900000000000000000_forced_edit_budget_selection_turntable_cross_section.png"),
        "face_detail_claim_allowed": False,
        "allowed_face_claim": ALLOWED_FACE_CLAIM,
        "summary": "V209 forces proposal-supported weak regions to receive connected edit budget. It is a diagnostic repair: pass still requires mentor visuals and hard controls.",
    }
    write_json(REPORTS / "V20900000000000000000_forced_edit_budget_selection_decision.json", decision)
...
def main() -> int:
    ...
    steps = int(os.environ.get("V20900_STEPS", "40"))
    max_points = int(os.environ.get("V20900_MAX_POINTS", "8192"))
    manifest = [train_case(row, steps=steps, max_points=max_points, device=device) for row in rows]
    write_csv(REPORTS / "V20900000000000000000_forced_edit_budget_selection_manifest.csv", manifest)
    compare(rows, created_at)
    write_json(
        REPORTS / "V20900000000000000000_runtime_environment.json",
        {"created_at": created_at, **runtime, "steps": steps, "max_points": max_points},
    )
    print(json.dumps({"created_at": created_at, "status": "V209_DONE", "device": str(device), "mentor_ready": False}, indent=2))
    return 0
