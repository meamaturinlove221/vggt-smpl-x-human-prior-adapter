...
BOARDS = REPO / "boards"
OUT_ROOT = OUTPUT / "V20420000000000000000_part_local_target_student"
TARGET_ROOT = OUTPUT / "V20410000000000000000_part_local_targets"
TRUE_CONFIG = "part_local_target_student_true"
...
def load_part_local_targets(case: str, n: int) -> tuple[np.ndarray, np.ndarray, dict[str, Any]]:
    target_path = TARGET_ROOT / case / "part_local_targets.npz"
    if not target_path.exists():
        raise FileNotFoundError(f"Missing V20410 part-local targets for {case}: {target_path}")
    target = load_npz(target_path)
    lock = np.asarray(target["visible_lock_mask"], dtype=bool)
    local = np.asarray(target["part_local_target_mask"], dtype=bool)
    if len(lock) != n:
        lock = np.interp(np.linspace(0, len(lock) - 1, n), np.arange(len(lock)), lock.astype(np.float32)) >= 0.5
        local = np.interp(np.linspace(0, len(local) - 1, n), np.arange(len(local)), local.astype(np.float32)) >= 0.5
    local = local & (~lock)
    info = {
        "v20410_target_npz": str(target_path),
        "visible_lock_ratio": float(np.mean(lock)),
        "part_local_target_ratio": float(np.mean(local)),
        "part_local_target_points": int(np.sum(local)),
    }
    return lock.astype(bool), local.astype(bool), info
...
def train_case(row: dict[str, str], steps: int, max_points: int, device: torch.device) -> dict[str, Any]:
    ...
    lock_mask, target_mask, target_info = load_part_local_targets(row["case"], len(baseline_points))
    preserve_mask = preserve_mask | lock_mask | (~target_mask)
    preserve_info.update(
        {
            "strict_preserve_ratio": float(np.mean(preserve_mask)),
            "part_local_edit_ratio": float(np.mean(target_mask)),
            **target_info,
        }
    )
    ...
    for step in range(steps):
        ...
        target_occ = torch.relu(0.55 - (out["occupancy"] * target_sel).sum() / target_sel.sum().clamp_min(1.0))
    ...
    np.savez_compressed(
        ...
        part_local_target_mask=target_mask.astype(bool),
        ...
        part_local_target_student=np.array(True),
...
def compare(rows: list[dict[str, str]], created_at: str) -> None:
    ...
    configs = [TRUE_CONFIG, V203_CONFIG, V202_CONFIG, V200_CONFIG, V197_CONFIG, V194_CONFIG, V192_CONFIG, V190_CONFIG, V187_CONFIG, V186_CONFIG, BASELINE_CONFIG, "same_topology_no_semantic", "shuffled_smpl_feature", "thickness_only_control"]
    for row in rows:
        case = row["case"]
        ...
        if true is None:
            failures.append({"case": case, "reason": "missing_part_local_target_student_true"})
            continue
        ...
        for cfg in [V203_CONFIG, V202_CONFIG, V200_CONFIG, V197_CONFIG, V194_CONFIG, V192_CONFIG, V190_CONFIG, V187_CONFIG, V186_CONFIG, BASELINE_CONFIG, "same_topology_no_semantic", "shuffled_smpl_feature", "thickness_only_control"]:
    ...
    write_csv(REPORTS / "V20420000000000000000_part_local_target_student_scores.csv", score_rows)
    ...
    if panels:
        compose(panels, 3, BOARDS / "V20420000000000000000_part_local_target_student_board.png")
    if first and config_path(first, TRUE_CONFIG).exists():
        pts, rgb, _body = load_config(first, TRUE_CONFIG)
        turn = [
            render_panel(pts, rgb, f"{first} part-local-target {label}", rot=rot)
            for label, rot in [
        ...
        compose(turn, 3, BOARDS / "V20420000000000000000_part_local_target_student_turntable_cross_section.png")
    decision = {
        "created_at": created_at,
        "status": "V20420_PART_LOCAL_TARGET_STUDENT_FAIL_CLOSED_CONTINUE" if failures else "V20420_PART_LOCAL_TARGET_STUDENT_PRECHECK_PASS_REQUIRES_MENTOR_VISUAL",
        "mentor_ready": False,
        "external_hard_block": False,
        "failures": failures,
        "score_csv": str(REPORTS / "V20420000000000000000_part_local_target_student_scores.csv"),
        "board": str(BOARDS / "V20420000000000000000_part_local_target_student_board.png"),
        "turntable_cross_section": str(BOARDS / "V20420000000000000000_part_local_target_student_turntable_cross_section.png"),
        "face_detail_claim_allowed": False,
        "allowed_face_claim": ALLOWED_FACE_CLAIM,
        "summary": "V20420 trains a conservative student with explicit V20410 part-local target masks. It remains fail-closed unless full-scene mentor visuals and hard controls pass.",
    }
    write_json(REPORTS / "V20420000000000000000_part_local_target_student_decision.json", decision)
...
def main() -> int:
    ...
    steps = int(os.environ.get("V20420_STEPS", "40"))
    max_points = int(os.environ.get("V20420_MAX_POINTS", "8192"))
    manifest = [train_case(row, steps=steps, max_points=max_points, device=device) for row in rows]
    write_csv(REPORTS / "V20420000000000000000_part_local_target_student_training_manifest.csv", manifest)
    compare(rows, created_at)
    write_json(
        REPORTS / "V20420000000000000000_runtime_environment.json",
        {"created_at": created_at, **runtime, "steps": steps, "max_points": max_points},
    )
    print(json.dumps({"created_at": created_at, "status": "V20420_DONE", "device": str(device), "mentor_ready": False}, indent=2))
    return 0
