...
BOARDS = REPO / "boards"
OUT_ROOT = OUTPUT / "V20000000000000000000_visible_baseline_locked_completion"
TRUE_CONFIG = "visible_baseline_locked_completion_true"
...
def build_visible_baseline_locked_completion(row: dict[str, str]) -> dict[str, Any]:
    ...
    _preserve, info = visible_preserve_mask(row, len(base_points))
    ...
    if len(conf) != len(base_points):
        conf = np.interp(np.linspace(0, len(conf) - 1, len(base_points)), np.arange(len(conf)), conf).astype(np.float32)
        weak = np.interp(np.linspace(0, len(weak) - 1, len(base_points)), np.arange(len(weak)), weak).astype(np.float32)
    # V198 showed baseline is already the most mentor-readable visible surface.
    # Preserve 85% of it and make the completion prove non-regression first.
    rank = 0.84 * conf - 0.16 * weak
    rank_preserve = rank >= np.quantile(rank, 0.15)
    keep_count = min(54000, max(50000, int(np.sum(rank_preserve))))
    ...
    # Candidate infill comes from V194/V192 trained outputs, but only in a small
    # low-confidence weak-region band. This is not a success shortcut: if the
    # locked surface is not visibly better than baseline/controls, it fails.
    ...
    near = float(np.quantile(dist, 0.10))
    far = float(np.quantile(dist, 0.38))
    if far <= near:
        far = near + 1e-4
    target_offset = float(np.quantile(dist, 0.24))
    weak_scale = max(float(np.quantile(weak_dist, 0.46)), 1e-6)
    ...
    score = 0.46 * offset_score + 0.47 * weak_score + 0.05 * body_bonus - 0.68 * far_penalty
    band = (dist >= near) & (dist <= far)
    ids = np.flatnonzero(band)
    target_infill = min(9000, max(5000, 60000 - len(preserved_points)))
    ...
    np.savez_compressed(
        ...
        visible_locked_infill_points=np.array(len(infill_points)),
        ...
        visible_baseline_locked_completion=np.array(True),
    ...
    return {
        ...
        "visible_locked_infill_points": int(len(infill_points)),
...
def compare(rows: list[dict[str, str]], created_at: str) -> None:
    ...
    for row in rows:
        ...
        if true is None:
            failures.append({"case": case, "reason": "missing_visible_baseline_locked_true"})
            continue
    ...
    write_csv(REPORTS / "V20000000000000000000_visible_baseline_locked_scores.csv", score_rows)
    ...
    if panels:
        compose(panels, 3, BOARDS / "V20000000000000000000_visible_baseline_locked_board.png")
    if first and config_path(first, TRUE_CONFIG).exists():
        ...
        compose(turn, 3, BOARDS / "V20000000000000000000_visible_baseline_locked_turntable_cross_section.png")
    decision = {
        "created_at": created_at,
        "status": "V20000_VISIBLE_BASELINE_LOCKED_FAIL_CLOSED_CONTINUE" if failures else "V20000_VISIBLE_BASELINE_LOCKED_PRECHECK_PASS_REQUIRES_MENTOR_VISUAL",
        "mentor_ready": False,
        "external_hard_block": False,
        "failures": failures,
        "score_csv": str(REPORTS / "V20000000000000000000_visible_baseline_locked_scores.csv"),
        "board": str(BOARDS / "V20000000000000000000_visible_baseline_locked_board.png"),
        "turntable_cross_section": str(BOARDS / "V20000000000000000000_visible_baseline_locked_turntable_cross_section.png"),
        "face_detail_claim_allowed": False,
        "allowed_face_claim": ALLOWED_FACE_CLAIM,
        "summary": "V200 locks the mentor-readable VGGT visible baseline surface and adds only small weak-region infill. It is a non-regression test, not a mentor-ready shortcut.",
    }
    write_json(REPORTS / "V20000000000000000000_visible_baseline_locked_decision.json", decision)
...
def main() -> int:
    ...
    manifest = [build_visible_baseline_locked_completion(row) for row in rows]
    write_csv(REPORTS / "V20000000000000000000_visible_baseline_locked_manifest.csv", manifest)
    compare(rows, created_at)
    write_json(
        REPORTS / "V20000000000000000000_runtime_environment.json",
        {
            "created_at": created_at,
            "selected_device": "not_required_deterministic_visible_baseline_locked_completion",
            "modal_required_for_final": False,
            "uses_modal_trained_inputs": True,
        },
    )
    print(json.dumps({"created_at": created_at, "status": "V20000_DONE", "mentor_ready": False}, indent=2))
    return 0
