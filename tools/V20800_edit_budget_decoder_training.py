...
BOARDS = REPO / "boards"
OUT_ROOT = OUTPUT / "V20800000000000000000_edit_budget_decoder_training"
PROPOSAL_ROOT = OUTPUT / "V20500000000000000000_learned_part_proposals"
TRUE_CONFIG = "edit_budget_decoder_true"
...
def train_case(row: dict[str, str], steps: int, max_points: int, device: torch.device) -> dict[str, Any]:
    ...
    keep_base = preserve
    connected_ids, quota_info = part_local_target_infill_ids(infill_points, infill_body, baseline_points, baseline_body, keep_base, target_mask, 60000)
...
def compare(rows: list[dict[str, str]], created_at: str) -> None:
    ...
    configs = [TRUE_CONFIG, V207_CONFIG, V206_CONFIG, V204_CONFIG, V203_CONFIG, V202_CONFIG, V200_CONFIG, V197_CONFIG, V194_CONFIG, V192_CONFIG, V190_CONFIG, V187_CONFIG, V186_CONFIG, BASELINE_CONFIG, "same_topology_no_semantic", "shuffled_smpl_feature", "thickness_only_control"]
    for row in rows:
        case = row["case"]
        ...
        if true is None:
            failures.append({"case": case, "reason": "missing_edit_budget_decoder_true"})
            continue
        ...
        for cfg in [V207_CONFIG, V206_CONFIG, V204_CONFIG, V203_CONFIG, V202_CONFIG, V200_CONFIG, V197_CONFIG, V194_CONFIG, V192_CONFIG, V190_CONFIG, V187_CONFIG, V186_CONFIG, BASELINE_CONFIG, "same_topology_no_semantic", "shuffled_smpl_feature", "thickness_only_control"]:
    ...
    write_csv(REPORTS / "V20800000000000000000_edit_budget_decoder_scores.csv", score_rows)
    ...
    if panels:
        compose(panels, 3, BOARDS / "V20800000000000000000_edit_budget_decoder_board.png")
    if first and config_path(first, TRUE_CONFIG).exists():
        pts, rgb, _body = load_config(first, TRUE_CONFIG)
        turn = [
            render_panel(pts, rgb, f"{first} edit-budget {label}", rot=rot)
            for label, rot in [
        ...
        compose(turn, 3, BOARDS / "V20800000000000000000_edit_budget_decoder_turntable_cross_section.png")
    decision = {
        "created_at": created_at,
        "status": "V208_EDIT_BUDGET_DECODER_FAIL_CLOSED_CONTINUE" if failures else "V208_EDIT_BUDGET_DECODER_PRECHECK_PASS_REQUIRES_MENTOR_VISUAL",
        "mentor_ready": False,
        "external_hard_block": False,
        "failures": failures,
        "score_csv": str(REPORTS / "V20800000000000000000_edit_budget_decoder_scores.csv"),
        "board": str(BOARDS / "V20800000000000000000_edit_budget_decoder_board.png"),
        "turntable_cross_section": str(BOARDS / "V20800000000000000000_edit_budget_decoder_turntable_cross_section.png"),
        "face_detail_claim_allowed": False,
        "allowed_face_claim": ALLOWED_FACE_CLAIM,
        "summary": "V208 adds explicit edit-budget and connectivity heads on top of V207 proposal conditioning. It remains fail-closed unless full-scene mentor visuals and hard controls pass.",
    }
    write_json(REPORTS / "V20800000000000000000_edit_budget_decoder_decision.json", decision)
...
def main() -> int:
    ...
    steps = int(os.environ.get("V20800_STEPS", "40"))
    max_points = int(os.environ.get("V20800_MAX_POINTS", "8192"))
    manifest = [train_case(row, steps=steps, max_points=max_points, device=device) for row in rows]
    write_csv(REPORTS / "V20800000000000000000_edit_budget_decoder_training_manifest.csv", manifest)
    compare(rows, created_at)
    write_json(
        REPORTS / "V20800000000000000000_runtime_environment.json",
        {"created_at": created_at, **runtime, "steps": steps, "max_points": max_points},
    )
    print(json.dumps({"created_at": created_at, "status": "V208_DONE", "device": str(device), "mentor_ready": False}, indent=2))
    return 0
