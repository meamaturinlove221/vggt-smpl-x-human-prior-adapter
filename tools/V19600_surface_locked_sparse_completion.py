...
BOARDS = REPO / "boards"
OUT_ROOT = OUTPUT / "V19600000000000000000_surface_locked_sparse_completion"
TRUE_CONFIG = "surface_locked_sparse_completion_true"
...
def build_sparse_completion(row: dict[str, str]) -> dict[str, Any]:
    ...
    # Candidate infill comes from V194 if present, otherwise from V192. It is
    # aligned and then clipped to a local band around preserved surface.
    ...
    score = -dist + 0.55 * np.exp(-weak_dist / max(np.quantile(weak_dist, 0.65), 1e-6)) + 0.06 * body_bonus
    band = dist <= max(float(np.quantile(dist, 0.42)), 1e-4)
    ...
    np.savez_compressed(
        ...
        sparse_infill_points=np.array(len(infill_points)),
        ...
        surface_locked_sparse_completion=np.array(True),
    ...
    return {
        ...
        "sparse_infill_points": int(len(infill_points)),
...
def compare(rows: list[dict[str, str]], created_at: str) -> None:
    ...
    for row in rows:
        ...
        if true is None:
            failures.append({"case": case, "reason": "missing_surface_locked_sparse_true"})
            continue
    ...
    write_csv(REPORTS / "V19600000000000000000_surface_locked_sparse_scores.csv", score_rows)
    ...
    if panels:
        compose(panels, 3, BOARDS / "V19600000000000000000_surface_locked_sparse_board.png")
    if first and config_path(first, TRUE_CONFIG).exists():
        ...
        compose(turn, 3, BOARDS / "V19600000000000000000_surface_locked_sparse_turntable_cross_section.png")
    decision = {
        "created_at": created_at,
        "status": "V19600_SURFACE_LOCKED_SPARSE_FAIL_CLOSED_CONTINUE" if failures else "V19600_SURFACE_LOCKED_SPARSE_PRECHECK_PASS_REQUIRES_MENTOR_VISUAL",
        "mentor_ready": False,
        "external_hard_block": False,
        "failures": failures,
        "score_csv": str(REPORTS / "V19600000000000000000_surface_locked_sparse_scores.csv"),
        "board": str(BOARDS / "V19600000000000000000_surface_locked_sparse_board.png"),
        "turntable_cross_section": str(BOARDS / "V19600000000000000000_surface_locked_sparse_turntable_cross_section.png"),
        "face_detail_claim_allowed": False,
        "allowed_face_claim": ALLOWED_FACE_CLAIM,
        "summary": "V196 keeps most visible VGGT baseline points and adds only sparse surface-locked infill. It remains fail-closed unless mentor visuals and hard controls pass.",
    }
    write_json(REPORTS / "V19600000000000000000_surface_locked_sparse_decision.json", decision)
...
def main() -> int:
    ...
    manifest = [build_sparse_completion(row) for row in rows]
    write_csv(REPORTS / "V19600000000000000000_surface_locked_sparse_manifest.csv", manifest)
    compare(rows, created_at)
    write_json(
        REPORTS / "V19600000000000000000_runtime_environment.json",
        {
            "created_at": created_at,
            "selected_device": "not_required_deterministic_surface_locked_infill",
            "modal_required_for_final": False,
            "uses_modal_trained_inputs": True,
        },
    )
    print(json.dumps({"created_at": created_at, "status": "V19600_DONE", "mentor_ready": False}, indent=2))
    return 0
