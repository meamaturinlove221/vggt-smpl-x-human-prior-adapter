...
APP_NAME = os.environ.get("VGGT_MODAL_V19200_APP_NAME", "vggt-v19200-upright-pose-frame-layout")
VOLUME_NAME = os.environ.get("VGGT_MODAL_V19200_VOLUME", "vggt-v19200-upright-pose-frame-output")
REMOTE_REPO = PurePosixPath("/workspace/repo")
REMOTE_OUT = PurePosixPath("/v19200_out")
...
@app.function(
    image=image,
    gpu=os.environ.get("VGGT_MODAL_V19200_GPU", "A10G"),
    cpu=4.0,
    memory=24 * 1024,
    timeout=8 * 60 * 60,
    volumes={str(REMOTE_OUT): volume},
)
def run_v19200(steps: int = 300, max_points: int = 8192) -> dict[str, Any]:
    import os
    import shutil
    import subprocess
    import sys
    ...
    env["V19200_STEPS"] = str(steps)
    env["V19200_MAX_POINTS"] = str(max_points)
    env["PYTHONUTF8"] = "1"
    env["PYTHONIOENCODING"] = "utf-8"
    proc = subprocess.run(
        [sys.executable, "tools/V19200_upright_pose_frame_layout.py"],
        cwd=repo,
        env=env,
        text=True,
        capture_output=True,
    ...
    copied: list[str] = []
    for rel in [
        "reports/V19200000000000000000_upright_pose_frame_training_manifest.csv",
        "reports/V19200000000000000000_upright_pose_frame_scores.csv",
        "reports/V19200000000000000000_upright_pose_frame_decision.json",
        "reports/V19200000000000000000_runtime_environment.json",
        "boards/V19200000000000000000_upright_pose_frame_board.png",
        "boards/V19200000000000000000_upright_pose_frame_turntable_cross_section.png",
    ]:
        src = repo / rel
        if src.exists():
            dst = out_root / rel
            ...
            copied.append(rel)
    matrix_src = repo / "output" / "V19200000000000000000_upright_pose_frame_layout"
    matrix_dst = out_root / "output" / "V19200000000000000000_upright_pose_frame_layout"
    if matrix_src.exists():
        if matrix_dst.exists():
            shutil.rmtree(matrix_dst)
        shutil.copytree(matrix_src, matrix_dst)
        copied.append("output/V19200000000000000000_upright_pose_frame_layout")
    volume.commit()
    return {
        "created_at": now(),
...
def main(steps: int = 300, max_points: int = 8192) -> None:
    result = run_v19200.remote(steps=steps, max_points=max_points)
