...
class EditBudgetProposalDecoder(CanonicalSurfelGraphOccupancyStudent):
    """Proposal-conditioned decoder with explicit part-local edit budgets.

    V207 injected proposal features before occupancy/residual decisions, but it
    could still choose a conservative baseline-near solution. V208 adds an edit
    budget head and uses that budget to bias weak, proposal-supported regions
    toward visible connected infill while preserving locked VGGT surface points.
    """
    ...
    def __init__(self, cfg: CanonicalSurfelGraphOccupancyConfig | None = None) -> None:
        ...
        self.edit_budget_head = nn.Linear(cfg.hidden_dim, 1)
        self.connectivity_head = nn.Linear(cfg.hidden_dim, 1)
    ...
    def forward(self, batch: Mapping[str, Tensor]) -> dict[str, Tensor]:
        ...
        edit_budget = torch.sigmoid(self.edit_budget_head(conditioned))
        connectivity = torch.sigmoid(self.connectivity_head(conditioned))
        ...
        seed = batch.get("proposal_seed")
        if seed is None:
            seed = torch.zeros_like(proposal)
        elif seed.ndim == 2:
            seed = seed.unsqueeze(-1)
        weak = batch.get("weak_score")
        if weak is None:
            weak = torch.zeros_like(proposal)
        elif weak.ndim == 2:
            weak = weak.unsqueeze(-1)
        edit_drive = proposal.clamp(0, 1) * (0.45 + 0.35 * seed.clamp(0, 1) + 0.20 * weak.clamp(0, 1))
        edit_drive = edit_drive * (1.0 - lock.clamp(0, 1))

        occupancy_logits = self.occupancy_head(h) + self.conditioned_occupancy_head(conditioned) * (0.35 + proposal)
        occupancy_logits = occupancy_logits + 2.20 * edit_budget * edit_drive + 0.85 * connectivity * edit_drive
        occupancy_logits = occupancy_logits - 1.55 * lock * nonregression
        ...
        local = torch.tanh(local_base + local_cond * (0.25 + proposal + 0.75 * edit_budget * edit_drive)) * self.cfg.max_residual
...
def smoke_test() -> dict[str, Any]:
    ...
    model = EditBudgetProposalDecoder(cfg)
    gen = torch.Generator()
    gen.manual_seed(207)
    ...
    return {
        ...
        "edit_budget_shape": list(out["edit_budget"].shape),
        "connectivity_shape": list(out["connectivity"].shape),
