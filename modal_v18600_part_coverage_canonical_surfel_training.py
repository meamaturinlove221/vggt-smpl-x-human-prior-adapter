...
APP_NAME = os.environ.get("VGGT_MODAL_V18600_APP_NAME", "vggt-v18600-part-coverage-canonical-surfel-training")
VOLUME_NAME = os.environ.get("VGGT_MODAL_V18600_VOLUME", "vggt-v18600-part-coverage-canonical-surfel-output")
REMOTE_REPO = PurePosixPath("/workspace/repo")
REMOTE_OUT = PurePosixPath("/v18600_out")
...
@app.function(
    image=image,
    gpu=os.environ.get("VGGT_MODAL_V18600_GPU", "A10G"),
    cpu=4.0,
    memory=24 * 1024,
    timeout=8 * 60 * 60,
    volumes={str(REMOTE_OUT): volume},
)
def run_v18600(steps: int = 300, max_points: int = 8192) -> dict[str, Any]:
    import os
    import shutil
    import subprocess
    import sys
    ...
    env["V18600_STEPS"] = str(steps)
    env["V18600_MAX_POINTS"] = str(max_points)
    env["PYTHONUTF8"] = "1"
    env["PYTHONIOENCODING"] = "utf-8"
    proc = subprocess.run(
        [sys.executable, "tools/V18600_part_coverage_canonical_surfel_training.py"],
        cwd=repo,
        env=env,
        text=True,
        capture_output=True,
    ...
    copied: list[str] = []
    for rel in [
        "reports/V18600000000000000000_training_manifest.csv",
        ...
        if src.exists():
            ...
            copied.append(rel)
    matrix_src = repo / "output" / "V18600000000000000000_part_coverage_canonical_surfel_training"
    matrix_dst = out_root / "output" / "V18600000000000000000_part_coverage_canonical_surfel_training"
    if matrix_src.exists():
        if matrix_dst.exists():
            shutil.rmtree(matrix_dst)
        shutil.copytree(matrix_src, matrix_dst)
        copied.append("output/V18600000000000000000_part_coverage_canonical_surfel_training")
    volume.commit()
    return {
        "created_at": now(),
...
def main(steps: int = 300, max_points: int = 8192) -> None:
    result = run_v18600.remote(steps=steps, max_points=max_points)
