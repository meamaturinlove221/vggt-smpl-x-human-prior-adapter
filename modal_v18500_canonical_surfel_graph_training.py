...
APP_NAME = os.environ.get("VGGT_MODAL_V18500_APP_NAME", "vggt-v18500-canonical-surfel-graph-training")
VOLUME_NAME = os.environ.get("VGGT_MODAL_V18500_VOLUME", "vggt-v18500-canonical-surfel-graph-output")
REMOTE_REPO = PurePosixPath("/workspace/repo")
REMOTE_OUT = PurePosixPath("/v18500_out")
...
image = (
    ...
    .add_local_dir(str(REPO / "output" / "V17300000000000000000_multishell_topology_decoder_training"), remote_path=str(REMOTE_REPO / "output" / "V17300000000000000000_multishell_topology_decoder_training"))
    .add_local_dir(str(REPO / "output" / "V18300000000000000000_part_pair_exclusion_training"), remote_path=str(REMOTE_REPO / "output" / "V18300000000000000000_part_pair_exclusion_training"))
...
@app.function(
    image=image,
    gpu=os.environ.get("VGGT_MODAL_V18500_GPU", "A10G"),
    cpu=4.0,
    memory=24 * 1024,
    timeout=8 * 60 * 60,
    volumes={str(REMOTE_OUT): volume},
)
def run_v18500(steps: int = 300, max_points: int = 8192) -> dict[str, Any]:
    import os
    import shutil
    import subprocess
    import sys
    ...
    env["V18500_STEPS"] = str(steps)
    env["V18500_MAX_POINTS"] = str(max_points)
    env["PYTHONUTF8"] = "1"
    env["PYTHONIOENCODING"] = "utf-8"
    proc = subprocess.run(
        [sys.executable, "tools/V18500_canonical_surfel_graph_training.py"],
        cwd=repo,
        env=env,
        text=True,
        capture_output=True,
    ...
    copied: list[str] = []
    for rel in [
        "reports/V18500000000000000000_training_manifest.csv",
        "reports/V18500000000000000000_canonical_surfel_scores.csv",
        "reports/V18500000000000000000_training_decision.json",
        "reports/V18500000000000000000_runtime_environment.json",
        "boards/V18500000000000000000_canonical_surfel_board.png",
        "boards/V18500000000000000000_canonical_surfel_turntable_cross_section.png",
    ]:
        src = repo / rel
        if src.exists():
            dst = out_root / rel
            ...
            copied.append(rel)
    matrix_src = repo / "output" / "V18500000000000000000_canonical_surfel_graph_training"
    matrix_dst = out_root / "output" / "V18500000000000000000_canonical_surfel_graph_training"
    if matrix_src.exists():
        if matrix_dst.exists():
            shutil.rmtree(matrix_dst)
        shutil.copytree(matrix_src, matrix_dst)
        copied.append("output/V18500000000000000000_canonical_surfel_graph_training")
    volume.commit()
    return {
        "created_at": now(),
...
def main(steps: int = 300, max_points: int = 8192) -> None:
    result = run_v18500.remote(steps=steps, max_points=max_points)
